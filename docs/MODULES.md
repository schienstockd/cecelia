...
`popType` selects which population type to list (`"flow"`/`"live"`/`"clust"`); behaviour uses `"live"` (cell pops with the derived `/_tracked`). Membership is **not** resolved in the widget — the Julia handler resolves label IDs via the gating engine.
```json
{ "key": "pops", "label": "Populations", "type": "popSelection", "popType": "live", "multiple": true, "acrossSegmentations": true, "default": [] }
```
