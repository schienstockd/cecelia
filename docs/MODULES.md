...
**`accepts` — the explicit pop_type allow-list (supersedes `popScope`).** A `popSelection` may instead declare **`"accepts": [...]`** — the exact pop_types the function takes, any of `live`/`flow` (cell gates), `clust` (cell clusters), `region` (spatial regions), `track` (per-track gates + the derived `_tracked` sets), `trackclust` (track clusters). This is the general form: it can offer **cells AND tracks in one picker** (`popScope` can't). A function that operates on cells but should accept *any* population — spatial analysis, region/cell clustering — declares **all** of them:
```json
{ "key": "pops", "label": "Populations", "type": "popSelection", "accepts": ["live", "clust", "region", "track", "trackclust"], "multiple": true, "default": [] }
```
Track-grained pops (`track`/`trackclust`/`_tracked`) selected into a **cell** function expand to their member cells. Because the selected value carries only the path (not its type), a consumer of a mixed-`accepts` picker MUST resolve membership with **`pop_df_multi`** (`population_manager.jl`) — it discovers each pop's type (`resolve_pop_type`), calls `pop_df` per type at `granularity=:cell`, and stacks/dedups. Do **not** hand-roll a single `get(params,"popType","flow")` default: that was the bug that hid tracked cells and returned empty for region/cluster pops. Annotation tasks that write `<ns>.cell.*` columns derive the namespace with **`pop_namespace`** (tracked source → `live`, else `flow`).

**Derive the segmentation from the pops — don't add a `valueNameSelection` dropdown.** A single-segmentation task should NOT pair a `popSelection` with a separate "Segmentation" dropdown: each picker value is already value_name-prefixed (`"B/qc"`, all-cells root `"B/"`), so the segmentation is implied by the pick. Resolve it with **`pops_value_name(pops)`** (`population_manager.jl`) — it drives which cells are read *and* where output/annotation is written. A separate dropdown is redundant and a footgun (pick a pop from segmentation B while the dropdown says A → the run silently resolves to zero cells). The spatial-analysis tasks all do this; `acrossSegmentations:false` keeps a picker to one segmentation. The all-cells "whole segmentation" case is the picker's per-segmentation `"all"` root (`"B/"`), which `population_accept_groups` always offers when `live` is accepted — no `includeRoot` flag needed.

Current users: `behaviour.hmm_states` / `behaviour.hmm_transitions` / `clustTracks.cluster` → `popScope:"tracks"`; `tracking.bayesian_tracking` → `popScope:"cells"` (gates cells to track — tracked pops would be nonsensical); all spatial-analysis functions + `clustPops.cluster` / `clustRegions.cluster` → `accepts:["live","clust","region","track","trackclust"]` (any population, resolved to cells via `pop_df_multi`).
