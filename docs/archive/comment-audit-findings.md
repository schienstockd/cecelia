...
- `app/src/tasks/chain.jl:23` — `ChainNode.params::Dict{String,Any}` is the on-node params bag; every task consumes it differently. Same boundary as scheduler/af_correct — the fix is per-task typed constructors, not a chain-level fix. **Partial:** the `cleanupImages/` family (af_correct, denoise, drift_correct, flow_register, smooth, stack_align) has landed the pattern — each task carries a `Base.@kwdef struct <Task>Params` + `parse_<task>_params(::AbstractDict)`, and `_run_task` reads only `p.field` accesses. Remaining task families to fan out: segment, editImages, importImages, exportImages, tracking, cluster{Pops,Tracks}, spatialAnalysis, misc.
