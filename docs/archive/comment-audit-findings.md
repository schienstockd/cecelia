...
- ✅ `af_qc_findings(per_channel)` in `app/src/tasks/cleanupImages/af_correct.jl` — **Resolved** via `struct AfChannelStats` + `parse_af_channel_stats(::AbstractDict)` at the JSON parse boundary. `af_qc_findings` now reads the struct's fields; a Python-side key rename shows up as a `Float64(get(...))` at parse rather than a silent default at each finding site.
- ✅ `af_combinations_for_python(params, raw)` in the same file — **Resolved** via `struct AfCombinationSpec` + `parse_af_combinations(::AbstractDict)`. Malformed entries are dropped explicitly (rather than silently degrading) and the resolver runs off the typed spec vector, so the frontend contract is one authoritative Julia type.
...
- ✅ `app/src/tasks/chain.jl:23` — `ChainNode.params::Dict{String,Any}`. **Resolved** across every task family: cleanupImages (#906), editImages (#909), tracking (#910), segment (#913), opticalFlow (#914), and the remainder bundle (importImages / exportImages / clust{Pops,Tracks,Regions} / behaviour / spatialAnalysis) — 42+ tasks total. Each task carries a `Base.@kwdef struct <Task>Params` + `parse_<task>_params(::AbstractDict)`, and `_run_task` reads only `p.field` accesses. `ChainNode.params` itself stays a `Dict{String,Any}` (composition + serialisation are unchanged); the drift risk moved from the read sites to the parsers, one per task, each pinned by a unit test. `testTasks/` skipped by design — they exist to exercise raw-Dict dispatch.
...
- ✅ `app/src/label_props.jl` — `write_categorical_obs(columns::AbstractVector; …)`. **Resolved** via `struct CategoricalObsColumn` + `_to_categorical_obs_column(::Any)` coercer at entry. Both the historical `(; name, labels, values)` NamedTuple and `AbstractDict` entries are still accepted; the coercer names the missing/wrong field in an entry-time error rather than dying deep in the Python subprocess.
- ✅ `app/src/label_props.jl` — `add_obs(lp, df)`. **Resolved** via `_assert_float_convertible(df)` at `add_obs` entry: a non-numeric column raises a named-column error that points at `write_categorical_obs`, instead of `Float64(v)` throwing a generic MethodError mid-`save!`. `Bool` deliberately refused (would silently coerce to 0/1); `missing`/`nothing` still allowed (they map to NaN at write).
...
- ✅ `api/src/gating_api.jl` — **Resolved** via shared `_require_ids(body)` returning `(pu, iu, err)` (err = 400 on missing / empty). The three raw `body["projectUid"]` / `body["imageUid"]` sites (`_persist_and_broadcast!`, `_history_step`, `api_gating_copy`) all route through it now, so a hand-rolled caller that skips `_gating_post` gets the same 400 as every other handler instead of a stack-traced KeyError.
- ✅ `api/src/notebooks_api.jl` — **Resolved** via `mutable struct PlutoServerState` (proc + starting + error + its own lock) with `_with_pluto_state_lock(f)` as the ONE mutation entry. The three server transitions (spawn / clear-starting / shutdown) now update all fields atomically under the struct's lock; `api_notebooks_status` reads the pair inside the lock so a mid-transition write cannot split them. The independent `_nb_build_proc` / `_nb_build_error` pair stays as Refs (different lifecycle from the server proc — used in one write and one read each; bundling them into the server state would couple unrelated concerns).
...
- ✅ **`app/src/config.jl`** — **landed.** Split into `app/src/config/{models,binaries,throttle,image_format}.jl`; root `config.jl` (221L) keeps the bootstrap + orchestration + include manifest. All names still live in the top-level `Cecelia` module (no export changes). test-pkg + test-api green.
