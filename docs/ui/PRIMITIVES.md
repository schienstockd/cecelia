...
| **Pick an arbitrary colour** for a thing (a population, an image channel) | `components/ColourPicker.vue` — a swatch you click, opening the palette + the native picker for anything outside it. `v-model` a hex; `palette` defaults to the house set (pass `CHANNEL_COLORMAP_OPTIONS`' hexes for image channels) | a second swatch-plus-popover, or `SwatchSelect` where the option's NAME is not worth the row width (it squeezed the viewer's channel names to one character each) |
| Pick from a NAMED set of colormaps, label included | `components/SwatchSelect.vue` | a bespoke swatch grid |
