...
| GET | `/api/tasks/history?projectUid[&limit]` | recent task runs across **all** images, newest first: `{count, history:[{imageUid,imageName,status,fun,valueName,at}]}` — aggregates each image's `runlog.json`. `limit` default 100. Read-only; backs the observer's `get_task_history`. (Attempt counts arrive with the per-node counter in a later slice.) |
