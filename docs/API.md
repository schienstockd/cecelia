...
| POST | `/api/napari/toggle-animation` | — | dock/undock **napari-animation**'s recorder "wizard" widget in the running viewer (`toggle_animation!` → bridge `toggle_animation`). `200 {ok,active}` (`active` = now shown); `400` if napari not running. Backs the viewer sidebar's movie-recorder toggle. |
