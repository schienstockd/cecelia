...
| GET/POST | `/api/lablog?projectUid` · `/api/lablog/append` · `/api/lablog/capture` | per-project **lab log** — an append-only markdown file `{proj}/lab-log.md` (cross-session AI+human analysis memory; see `docs/ai-assist/LAB-LOG.md`). **GET** → `{content, entries:[{date,author,lines,raw}] newest-first, mtime}`. **POST /append** `{projectUid, author, lines: string \| [string]}` → appends one `## <date> [<author>]` block; the server **injects the date + author tag** (callers never send the header) and the write is **append-only** + project-lock-guarded (`with_transaction`). **POST /capture** `{projectUid}` → appends an auto-generated `[Cecelia]` digest of run-log activity since the last capture (idempotent via a `settings/lab-log-context.json` cutoff); `{ok, captured, block, entries}` with `captured=false` when there's nothing new. `400` empty/ missing author; `404` unknown project. |
