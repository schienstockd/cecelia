...
| GET | `/api/movies/meta?projectUid&name` | the full registry entry for one movie, `{name, entry}` — **including** the saved generation config the list omits. `entry` is `{}` for a movie recorded before the registry, or one nothing has been set on. |
