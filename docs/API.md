...
| GET · POST | `/api/tasks/custom-modules` · `/api/tasks/custom-modules/reload` | user drop-in modules + plugins: `{dir, modules:[{path,plugin,status,error}], plugins:[{name,dir,version,description,homepage,categories,error,warning}], clashes:[{funName,path,plugin,tier,winner,winnerTier}], categories}`; `reload` returns the same payload plus `{loaded,skipped,removed,failed}`. Both scans (here and `/api/tasks/definitions`) enumerate the modules tree through the ONE helper `Cecelia.user_task_specs` — they used to hand-roll a one-level `readdir` each, which is why a plugin's task could register and run with no form. `clashes` is separate from `modules` on purpose: a `fun_name` collision is **not** a load failure (the file `include`s fine, it just loses the name to a higher tier — built-in > hand-dropped > plugin), so it would report `ok` and the task would simply be missing from the UI with nothing saying why. `warning` is the advisory `requiresCecelia` mismatch, skipped entirely on a `"dev"` build. See `docs/CUSTOM_MODULES.md`, `docs/todo/PLUGINS_PLAN.md` |
