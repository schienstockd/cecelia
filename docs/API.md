...
| GET | `/api/images/compression?projectUid&imageUid` | How each image VERSION is encoded on disk: `{versions: {valueName: {label, codec, level, shuffle} | null}}`. Read from each store's level-0 `.zarray` (metadata only, no pixels — same sanctioned carve-out as `/geometry`, `api/src/image_geometry.jl`). `label` is the name Settings → Storage uses for that codec so the two surfaces agree; a codec outside `IMAGE_COMPRESSOR_CHOICES` gets a descriptive label instead (`blosc/lz4-9`). Matching includes the WRAPPER — a blosc-wrapped unshuffled zstd is not the bare `zstd` choice. `null` for a version whose store is missing or unreadable, so the caller can show `—` rather than drop the row. EVERY version in one call: the consumer (the image-metadata modal) lists them all. |
