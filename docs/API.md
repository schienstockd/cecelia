...
| POST | `/api/projects/boards` | `projectUid, boards:{tabs,activeId,nextId,layouts}, version` | debounced **autosave** of the /analysis boards → `settings/analysisBoards.json`. Board images are sidecar files (below), not inline, so the JSON stays small. **Optimistic concurrency**: echo the `version` you last read; a stale one is rejected **409** `{code:"stale_version", boards}` (two browser tabs used to clobber each other silently). On success returns the new `version` and broadcasts `boards:changed`. Mirrors `/projects/canvases`; replaces the old `/projects/save` + manual save button |
| GET | `/api/projects/boards` | `?projectUid` | the boards document + its `version` on its own — the cheap reload behind a 409'd write and the `boards:changed` broadcast, so neither has to re-run a whole project load. Same path as the POST, different method |
...
| GET/POST | `/api/lablog?projectUid` · `/api/lablog/{append,capture,dismiss}` | per-project **lab log** — an append-only markdown file `{proj}/lab-log.md` (cross-session AI+human analysis memory; see `docs/ai-assist/LAB-LOG.md`). **GET** → `{content, entries:[{date,author,lines,raw}] newest-first, dismissed:[entryId…], imageNames:{uid→name}, mtime}` (`dismissed` = entry ids hidden from the panel view; `imageNames` = current uid→name map — the log stores **stable image UIDs**, the panel's "Show names" toggle resolves them to names on demand). **POST /append** `{projectUid, author, lines: string \| [string]}` → appends one `## <date> [<author>]` block; server **injects the date + author tag** (callers never send the header); **append-only** + project-lock-guarded (`with_transaction`). **POST /capture** `{projectUid}` → appends an auto-generated `[Cecelia]` digest of net activity since the last capture, **grouped by module category** (task-manager tags) and **collapsed across images**; idempotent via a `settings/lab-log-context.json` snapshot; `{ok, captured, block, entries}`. **POST /dismiss** `{projectUid, id, dismissed}` → hide/un-hide one entry from the panel view → `settings/lab-log-dismissed.json` (config sidecar; the log file is never edited); `{ok, dismissed}`. `400` empty/missing field; `404` unknown project; **`409`** when the author is tagged `LabArchives` on a project with no linked notebook — that tag is a provenance claim, and "there is no notebook to have come from" is the one part of it the server can verify. |
| POST | `/api/sets/{create,rename,delete}` | set CRUD. `rename` `{projectUid,setUid,name}` → `{uid,name}` is **display-only**: a set's identity is its uid (the dir under `{proj}/1/`), so no data moves and nothing else is told — images attach by uid, and per-set UI settings are uid-keyed. The name is **trimmed before validation** (whitespace-only → `400`, and `" x "` cannot slip a duplicate past the check). **`409`** when another set already has the name — the guard is the model's (`set_name_taken`, so a REPL caller and the `newSetName` paths on move/copy get it too) and these routes only map it to the code `chains/rename` already uses; renaming a set to its own name is a `200` no-op. **`create` behaves identically** (it previously neither trimmed nor checked). A caller that means "the set called X, making it if needed" resolves first and reuses — see `api_images_move`. |
...
| GET | `/api/objects/find?q=[&limit=]` | **Which project an object is in** — the only read that does not start from a `projectUid`, and the reason the others can. `q` is matched as a **uid first** (exact, case-sensitive; image, set or project — the metadata dir is `{proj}/1/{uid}`, so it costs one `isfile` per project and loads only the owning one; that stat is a **pre-filter only** — the match itself is the string comparison, because macOS/Windows filesystems are case-insensitive and would otherwise resolve a wrong-case uid to a real object), and **only if nothing matched** as a case-insensitive **substring of names** (which loads every project's `ccid.json` files — no pixel data). Returns `{query, matchedBy:"uid"\|"name", count, truncated, matches:[{kind:"image"\|"set"\|"project", uid, name, projectUid, projectName, …}]}` — plus `setUid`/`setName`/`status`/`included` on an image and `imageCount` on a set. Names are not unique, so a name query can return several matches across projects; `limit` (default 50, max 500) caps them and sets `truncated`, which callers must surface rather than presenting a trimmed list as complete. An object whose dir exists but that no set claims is still reported (with empty `setUid`), because "in this project, unattached" is the answer to where it lives. Read-only, no `lastOpenedAt` bump — same guarantee as `/api/images`. Backs the observer's `find_object`; before it, resolving a uid quoted in a note meant calling `/api/images` for every project until one matched. |
...
| GET | `/api/tasks/history?projectUid[&limit]` | recent task runs across **all** images, newest first: `{count, history:[{imageUid,imageName,status,runStatus,fun,valueName,at,params}]}` — aggregates each image's `runlog.json`. `status` = the image's current status; `runStatus` = that run's outcome, an OPEN set: `done` · `failed` · `cancelled` · `interrupted` (its process died mid-run) · `running` (live now); legacy entries → `"done"`. The last three mean no output was produced — never attribute a result to one. See `docs/SCHEDULER.md` → *a run is logged twice*. `params` = the params that run used (the tuning trail; `{}` on legacy entries) — lets the observer suggest a param adjustment on an outlier. `limit` default 100. Read-only; backs the observer's `get_task_history`. |
...
| GET | `/api/profiles` | **View profiles** — the curated-sidebar definitions dropped in under `<config_dir>/profiles/*.json`: `{dir, profiles:[{id,label,items}], errors:[{file,error}]}` (`Cecelia.read_view_profiles`). `items` is an ORDERED list of route paths; `id` is the filename stem. Validates **shape only** — the server does not know the route table (that is `frontend/src/main.ts`), so an item naming a route that no longer exists comes back as-is and the frontend resolves + reports it (`utils/viewProfiles.ts`). A broken file lands in `errors`, never as a 500: one typo must not blank someone's sidebar. The *selection* is NOT here — it is per-user (`cc.viewProfile` in `localStorage`). See `docs/todo/VIEW_PROFILES_PLAN.md`. |
| POST | `/api/profiles/save` | `{label, items[, id]}` → `{profile:{id,label,items}}` \| `400 {error}`. Create or overwrite one profile via `write_json_atomic`. The **id is derived from the label** (`safe_name_part`) so the user types a name, never a filename; pass `id` to rename a profile's label while KEEPING its id, or an active selection would break under the user. Validated by the same `parse_view_profile` the reader uses, so writer and reader cannot disagree about what a valid profile is — an empty `items` is a `400`, not an empty menu. |
| POST | `/api/profiles/delete` | `{id}` → `{deleted:Bool}`. `false` when it was already gone (not an error). The id is sanitised, so a request cannot escape the profiles dir. |
...
| GET | `/api/movies?projectUid` | list the project's rendered `.mp4`s under `{project}/movies/` for the **Movies** player (`/movies`, `MoviesModule.vue`): `{movies:[{name,size,mtime,displayName,starred,tags,producedBy,imageUid,channels,suffix,hasConfig,configKind,configStale}]}`, newest-first. `imageUid` is which image the movie is of and `channels` the channels it shows — both banked by the recorder, and both **back-filled** out of the saved config for movies that predate the fields (the single recorder has always banked its `imageUid` there; a batch's channel map sits under `config`, the viewer's under `look`). A batch config's `imageUids` is the whole selection, not this file's image, so it is deliberately not a fallback — the Movies page resolves those from the filename instead. `suffix` is the user's RAW "name" addition for that recording (not the `_sanitised` fragment in the filename), banked so the next recording can offer it back — `useMovieSuffixes` feeds the recorder's suggestion list from it. It is stored rather than parsed because the filename also carries uid and attribute parts with nothing marking where the suffix begins; `""` for movies recorded before the field existed. Empty list (not 404) when the folder doesn't exist yet. The registry fields come from `settings/movies.json` (`movies_with_meta` in `movies_api.jl`), which the same pass **reconciles**: an entry with no file is pruned, and one older than its file is flagged `configStale` (a re-record replaced the bytes under it). The saved **config** is deliberately not here — a keyframe config is large and the list renders none of it. Bytes are streamed separately by `/api/movies/file`. |
| GET | `/api/movies/meta?projectUid&name` | the full registry entry for one movie, `{name, entry}` — **including** the saved generation config the list omits. `entry` is `{}` for a movie recorded before the registry, or one nothing has been set on. This is what the **edit** action reads (`frontend/src/utils/movieRestore.ts`): the config comes back **verbatim**, nesting intact, because the reader takes it apart field by field. Its keys are a cross-language contract with `handle_movie_record`/`handle_movie_batch` (`sockets.jl`) that nothing type-checks — pinned by *"API: movie config banks what the edit page reads"*. |
| POST | `/api/movies/meta` | `{projectUid, name\|names, displayName?, starred?, tags?, addTags?, removeTags?}` → set the user-owned metadata on one movie **or many**. Every field is **optional and absent means "leave alone"**, so the page's controls patch independently. `tags` REPLACES; `addTags`/`removeTags` are set operations that leave a movie's other tags in place — categorising a selection must not wipe what the individual movies carry. `displayName` is a label only (the file is never renamed, `docs/todo/MOVIE_MANAGEMENT_PLAN.md` Decision 2) and is **ignored for a multi-movie call**, since one label across a selection only makes them indistinguishable. Returns `{names, rejected}`. **Bulk is ONE call, not a client loop**: the registry is a single JSON file, so N requests rewrite it N times and any two in flight lose one side's edit. `404` when none of the names is a movie on disk. |
| POST | `/api/movies/delete` | `{projectUid, name\|names}` → delete the `.mp4`(s) and their registry entries, in one pass. A movie is not an image: there are no version/label/analysis scopes to pick between, so this is a plain delete behind the page's confirm. Returns `{deleted, rejected}` — one locked file doesn't abandon the rest half-done. The `[A-Za-z0-9._-]+.mp4` name guard is what keeps it inside the movies dir. |
...
| GET | `/api/observer/briefing?projectUid` | **READ-ONLY**, project-level: the observer **session briefing** — `{projectUid, projectName, imageCount, flagged:[{uid,name,worst,findings:[{level,short}]}], recentLabLog:[{date,author,summary}]}`, plus `labarchives:{notebookName,syncedAt,sections:[heading],gaps}` **when the project has an ELN sidecar** (the key is omitted otherwise, so "not linked" stays distinct from "linked but empty"). `flagged` = images with a warn/fail QC finding (same source as the image table, `all_qc_docs`); `recentLabLog` = last 7 days, newest-first. The startup context a Chat-to-Claude session pulls first. `session_briefing`, Observer Phase 2 §2. |
| GET | `/api/mcp/connections` | **READ-ONLY**, machine-level (no project): every MCP server registered in the user's Claude config — `{configPath, present, connections:[{name,scope,dir,transport,ours}]}`. Generic (enumerates what's there, so a new connector needs no change here). **It cannot see claude.ai ACCOUNT connectors** (LabArchives): absence is not evidence of "disconnected". Backs Settings → MCP connections. `mcp_connections`. |
| GET | `/api/observer/labarchives?projectUid` | **READ-ONLY**, project-level: the LabArchives context sidecar IN FULL + `gaps` — `{present, readable, source:{notebookId,notebookName,url,pageIds}, syncedAt, sections:[{heading,lines,sourceDate,url}], cohort:[{attr,value,n}], gaps:[{attr,value,declared,present}]}`. `gaps` is derived on every read (declared cohort vs live image attributes), never stored. `read_la_doc`/`la_gaps`. |
| POST | `/api/observer/labarchives/set` | **REPLACES** the sidecar. Body `{projectUid, source, sections, cohort, syncedBy}`. The writer bounds sections/lines/length — the caller does not choose the shape. Cecelia holds no LabArchives credentials by design, so this is the only way ELN context arrives (Claude reads the ELN through the user's own connector). Broadcasts `lab_log_updated`. Never touches the lab log. `write_la_doc!`. |
...
| GET | `/api/tracking/issues` | `{projectUid, imageUid, valueName?, gapFrames?, gapSteps?, jumpFactor?, jumpQuantile?, minLen?, limit?=100}` | `{valueName, tracked, nTracks, stepScale, timeStep, total, counts:{kind→n}, issues:[{kind:"gap"\|"jump"\|"short"\|"duplicate", op, trackIds, atT, centroid, severity, reason}], paths}` — the **correction worklist**: what looks wrong in a tracking result, ranked, each candidate carrying the op that would fix it (applied by `tracking.correct`, never here — this route is read-only). Thresholds are relative to the image's own median step (`stepScale`), not µm. `paths` is the geometry of the candidates' tracks, in the shape below, so a row can draw the thing it is asking about in one request. `limit` caps the candidates; `total` says how many there were. |
| GET | `/api/tracking/paths` | `{projectUid, imageUid, valueName?, colorBy?, limit?=500}` | `{valueName, tracked, total, shown, timeStep, stepScale, colorBy, colorKind:"numeric"\|"categorical"\|"none", values:{trackId→value}, paths:{trackId→{t:[…], x:[…], y:[…], label:[…]}}}` — every track's polyline **in µm**, time-ordered, for the `trackPaths` plot (paths / star / rose). `colorBy` is any per-track column, resolved exactly as a track-grained gating axis is (`track_cell_measures`); a column this image lacks comes back as `colorBy:""` rather than an error. The list of columns to offer is NOT returned here — the picker reads `/api/gating/channels?popType=track`, so there is one vocabulary. Capped by track LENGTH, longest first; `total` vs `shown` is what the plot reports as not-drawn. |
...
|---|---|---|---|
| POST | `/api/napari/set-z-view` | `show3D,zSlice?` | how much of the z stack is shown: `show3D` renders the WHOLE stack volumetrically (`ndisplay=3`), otherwise 2D at `zSlice` (omit/`null` = keep the slice showing). ONE switch for the image AND the mask layers — napari's `Labels.projection_mode` accepts only `'none'`, so a mask cannot be flattened over z and "the whole stack" for it can only mean the 3D render. A 2D image refuses 3D (same guard as `open`'s `show3D`) and an out-of-range slice is clamped; the reply reports the state actually reached: `200 {ok,ndisplay,z}`; `400` if napari not running |
...
| GET | `/api/optical-flow/models` | — | the optical-flow model vault: `{dir, models:[{name,label,stem,bytes,modified,hasManifest,manifest}]}`. Enumerated by `Cecelia.list_coastal_models`, the SAME function that fills the segmenter's Model dropdown — the manager must not grow a second listing that can disagree with the picker. Machine-local and shared across projects, so it does **not** travel with a `.ccbundle`. `api_optical_flow_models` |
| POST | `/api/optical-flow/rename` | `name,newName` | rename a vault model. Moves BOTH files — `<name>.pt` and its `<name>.json` manifest — because an orphaned `.pt` silently falls back to coastal's default metric set and feeds the model misaligned channels. `409` on a name collision, `400` on a path separator. `api_optical_flow_rename` |
| POST | `/api/optical-flow/delete` | `name` | remove a vault model and its manifest. `api_optical_flow_delete` |
| POST | `/api/optical-flow/inspect` | `projectUid,imageUid,valueName,cellChannels,t,z?,colormap,regionSize?,temporalScales?,model?` | the two flow canvas plots, ONE route: metric planes with no `model`, the model's probability map with one. `{planes:[{name,png}], metricKeys, temporalScales?, region}` as base64 PNGs rendered server-side, or `202 {starting:true}` while the preview worker imports torch. `regionSize` is a **centred crop** in px per axis (default `FLOW_INSPECT_MAX_PX` = 512) — the whole frame is 36.3 MB and 8.2 s on a 1044×1102 movie against 9.2 MB and 2.4 s at 512, and it used to exceed the websocket frame cap outright. `z` null means the middle plane, resolved here rather than passed through. Not `api_preview_run`: a canvas plot has no napari coupling and must work with the viewer closed, so it resolves the store from `ccid.json`. `api_optical_flow_inspect` |
...
| POST | `/api/napari/centre` | `pos:[…]` (µm, centroid order), `tp?`, `zoom?` | fly the viewer to a point and frame — `centre!` in `app/src/napari.jl`. Used by the track-correction worklist's "Show", so a candidate row and the viewer agree on "here". `200 {ok}`; `400` if napari not running |
