...
| GET | `/api/storage/summary?projectUid` | Settings **storage box** (`api/src/storage_api.jl`). Walks every image store (expensive — the frontend calls it on a "Scan" button, not on open) + `diskstat`: `{diskTotal, diskAvailable, imageBytes, reclaimableBytes, reclaimable:[{imageUid,name,setUid,bytes,activeVersion}]}` (`imageBytes` = image OME-ZARR versions only, NOT labels/other task-dir data). `reclaimable` = images whose original `default` import can be freed because a corrected variant is active (`reclaimable_default`, biggest first). |
| POST | `/api/storage/reclaim` | `{projectUid, imageUids:[…]}` → free each image's original `default` import, keeping the active corrected variant (shared safe-primary `remove_image_version!`). `{ok, freedBytes, reclaimed:[uid]}`; ineligible/stale uids are skipped (never un-imported). `400` empty `imageUids`. |
...
| GET | `/api/pools`, `/api/tasks/definitions` | pools, task specs |
