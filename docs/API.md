...
| GET | `/api/movies?projectUid` | list the project's rendered `.mp4`s under `{project}/movies/` for the **Movies** player (`/movies`, `MoviesModule.vue`): `{movies:[{name,size,mtime,displayName,starred,tags,producedBy,hasConfig,configKind,configStale}]}`, newest-first. Empty list (not 404) when the folder doesn't exist yet. The registry fields come from `settings/movies.json` (`movies_with_meta` in `movies_api.jl`), which the same pass **reconciles**: an entry with no file is pruned, and one older than its file is flagged `configStale` (a re-record replaced the bytes under it). The saved **config** is deliberately not here — a keyframe config is large and the list renders none of it. Bytes are streamed separately by `/api/movies/file`. |
