...
| GET | `/api/movies?projectUid` | list the project's rendered `.mp4`s under `{project}/movies/` for the **Movies** player (`/movies`, `MoviesModule.vue`): `{movies:[{name,size,mtime,displayName,starred,tags,producedBy,imageUid,channels,hasConfig,configKind,configStale}]}`, newest-first. `imageUid` is which image the movie is of and `channels` the channels it shows — both banked by the recorder, and both **back-filled** out of the saved config for movies that predate the fields (the single recorder has always banked its `imageUid` there; a batch's channel map sits under `config`, the viewer's under `look`). A batch config's `imageUids` is the whole selection, not this file's image, so it is deliberately not a fallback — the Movies page resolves those from the filename instead. Empty list (not 404) when the folder doesn't exist yet. The registry fields come from `settings/movies.json` (`movies_with_meta` in `movies_api.jl`), which the same pass **reconciles**: an entry with no file is pruned, and one older than its file is flagged `configStale` (a re-record replaced the bytes under it). The saved **config** is deliberately not here — a keyframe config is large and the list renders none of it. Bytes are streamed separately by `/api/movies/file`. |
