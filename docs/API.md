...
| POST | `/api/movies/meta` | `{projectUid, name, displayName?, starred?, tags?}` → set the user-owned metadata. Every field is **optional and absent means "leave alone"**, so the page's three controls patch independently. `displayName` is a label only — the file is never renamed (`docs/todo/MOVIE_MANAGEMENT_PLAN.md` Decision 2) — and clearing it falls back to the file name. `tags` are free-form, deduped and capped. `400` on a name outside `[A-Za-z0-9._-]+.mp4`, `404` when no such movie is on disk. |
| POST | `/api/movies/delete` | `{projectUid, name}` → delete ONE `.mp4` and its registry entry. A movie is not an image: there are no version/label/analysis scopes to pick between, so this is a plain delete behind the page's confirm. Same name guard, which is also what keeps it inside the movies dir. |
