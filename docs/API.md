...
| GET | `/api/qc/cohort?projectUid&setUid&funName[&valueName][&threshold]` | recompute + return the **cohort QC summary** for one (task, output) across a set's *included* images: `{funName, valueName, nIncluded, metrics:{<key>:{n,median,mad,mean,sd,threshold,outliers:{uid:{value,z}}}}}`. Aggregates the per-image counts banked by `write_qc` (segment/measure/tracking); `funName` must be a known metric producer (`COHORT_METRICS`), else 400. Outliers use a **robust modified z-score** (median/MAD, Iglewicz–Hoaglin) so a clear anomaly flags even at n=3 and one bad image can't mask itself; `z` is that score, `threshold` its cutoff (default 3.5). `valueName` default `"default"`. Advisory (never blocks); recomputes from current data (no stale state) and also writes the sidecar `{proj}/1/{set_uid}/qc/cohort/{funName}/{valueName}.json`. Read-only w.r.t. task state; feeds the observer's cohort view + morning summary. |
