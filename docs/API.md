...
| GET/POST | `/api/lablog?projectUid` · `/api/lablog/append` · `/api/lablog/capture` · `/api/lablog/tune` | per-project **lab log** — an append-only markdown file `{proj}/lab-log.md` (cross-session AI+human analysis memory; see `docs/ai-assist/LAB-LOG.md`). **GET** → `{content, entries:[{date,author,lines,raw}] newest-first, tuning:{entryId→"up"\|"down"}, mtime}`. **POST /append** `{projectUid, author, lines: string \| [string]}` → appends one `## <date> [<author>]` block; the server **injects the date + author tag** (callers never send the header) and the write is **append-only** + project-lock-guarded (`with_transaction`). **POST /capture** `{projectUid}` → appends an auto-generated `[Cecelia]` digest of net activity since the last capture (idempotent via a `settings/lab-log-context.json` snapshot); `{ok, captured, block, entries}`, `captured=false` when nothing new. **POST /tune** `{projectUid, id, vote}` (`vote ∈ up\|down\|""`) → set/clear a **Tuning**-mode rating (entry-type useful/noise) in the `settings/lab-log-tuning.json` sidecar — config, *not* the log; `{ok, tuning}`. `400` empty/missing field; `404` unknown project. |
