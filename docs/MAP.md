...
| Add or change a cohort-comparable metric | `app/src/qc_cohort.jl` (`COHORT_METRICS`). A task that banks QC without an entry needs `# COHORT-EXEMPT: <reason>` in its `.jl` — enforced by `cohort-metrics ratchet` in `app/test/suite.jl`; rule in [`docs/MAINTAINABILITY.md`](MAINTAINABILITY.md) → *Cohort-comparable metrics* |
| Open a zarr store / parse OME-XML from Python | Only through `python/cecelia/utils/zarr_utils.py` + `ome_xml_utils.py`. Rule + ratchet in [`docs/MAINTAINABILITY.md`](MAINTAINABILITY.md) → *Image / OME-ZARR access*; enforced by `test_zarr_access_convention.py` (Python) and `zarr-access ratchet` in `app/test/suite.jl` (Julia — only `api/src/image_render.jl` may `using Zarr`) |
| Collapse a task's advanced params behind a section | `type: "section"` with `collapsed: true` in the task's `.json` — see `segment/cellpose.json`. Rule + ratchet in [`docs/MAINTAINABILITY.md`](MAINTAINABILITY.md) → *Task JSON — collapse advanced params*; enforced by `test_task_json_convention.py` |
