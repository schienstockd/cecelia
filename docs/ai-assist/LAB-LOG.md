...
- **Snapshot-diff, not an edit log.** `capture_context!(proj)` (`app/src/lab_log_context.jl`) reports the **net change since the last capture** — never per-edit keystrokes. A gating session with 50 polygon nudges that ends with one changed gate is *one* line; nudges that cancel out are nothing. Snapshots + a run-log cutoff live in `settings/lab-log-context.json`. Route: `POST /api/lablog/capture`. Three sources folded into one dated digest:
  ...
  - **Populations** — a per-pop fingerprint diffed against the last snapshot → `Populations img-3: added CD8+; gate changed on CD3; definition changed on clust_a`. The fingerprint hashes each pop's **whole membership definition** (gate *and* filters/cluster-membership/any future field — generic reflection over the struct, minus cosmetic fields), so it catches gate edits **and** filter/cluster-population edits (`clust`/`trackclust` are gateless, filter-defined). "gate changed" for gate pops, "definition changed" for filter pops. Read straight from the gating JSON files; no per-action trigger.
