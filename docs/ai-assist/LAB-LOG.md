...
- **Muting ("mute this kind").** In Tuning mode the panel shows a mute chip per category; muting one drops that category from *future* digests (`settings/lab-log-mutes.json`, `POST /api/lablog/mute`). Snapshots still advance while muted, so unmuting reports only future change, never a backlog. Config, not the log.
