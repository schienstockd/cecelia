...
- **Images are referenced by stable UID, not name.** Every image reference the digest writes (the `on N images (…)` list, the ↳ detail lines, population/exclusion lists, the cohort-check summary) is the image's **UID** — names change, UIDs don't, and UIDs are compact. The panel has a **"Show names"** toggle (`labLogShowNames`, default off — in the toolbar's Cecelia control group, next to Auto) that swaps each known UID for its *current* name at render time (resolved against the live `imageNames` map from `GET /api/lablog`) — a display-only substitution, so a rename shows through with no rewrite of the append-only log.
