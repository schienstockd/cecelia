...
- **Muting ("mute this kind").** In Tuning mode the panel shows a mute chip per category; muting one drops that category from the digest (`settings/lab-log-mutes.json`, `POST /api/lablog/mute`). Because the day's block is regenerated from source each capture, muting/unmuting takes effect on the *current* day's block immediately — unmute within a day and that day's activity in the category reappears; across days it doesn't (past days' blocks are immutable, and a category muted all of yesterday simply never entered yesterday's block). Config, not the log.
...
- **Tuning mode → judges the ENTRY TYPE → config, not the log.** 👍/👎 records "this *kind* of auto-entry is useful/noise" to the `settings/lab-log-tuning.json` sidecar (keyed by a frontend-computed stable entry id; `POST /api/lablog/tune`). Clicking the active vote clears it; a rated entry keeps its thumb visible. Tuning mode also shows a **mute chip per module category** ("mute this kind") — muting drops that category from future digests (`settings/lab-log-mutes.json`; see the *App-generated context* muting bullet). Neither pollutes the science record.
