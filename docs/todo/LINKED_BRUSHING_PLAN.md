...
- **B. Point-source (SHIPPED for boxplot 2026-09-23).** Adds a `pointIds` array parallel to
  ...
  - **Drag = freeform lasso**, **shift+drag = axis-aligned rectangle** (bulk selection). Lasso
    is the default gesture because a freeform loop is how "select this cluster" reads on a
    jitter cloud; rect stays for axis-aligned pulls (e.g. everything above y=X). Both draw a
    dashed SVG overlay; on release, every `.cc-brush-dot circle` whose center falls inside is
    collected (rect: bounding-box test; lasso: even-odd point-in-polygon), grouped by
    (imageUid, valueName), and dropped into the bag. Viewer mirror picks the preferred image
    (open viewer image if in scope, else the panel's own, else the largest hit group).
