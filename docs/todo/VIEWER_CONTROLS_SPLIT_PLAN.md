...
| P7 | WebGPU-native tracks (per-vn selection + ribbons) | **in progress** 2026-08-26 | Per-vn cell-track ribbons: DONE — `rebuildOverlays` gates `buildTrackBuffer` on `settings.getTrackVisibility(imageUid, [payload.valueName])`, so the panel's per-segmentation "directions" eye finally hides tracks in the WebGPU viewer instead of only touching napari. Default OFF preserved. **Remaining**: gated-track-pop ribbons (`toggleGatedTracks`, coloured by pop) and track-cluster ribbons (`toggleTrackclust`, coloured by cluster) — both need a new renderer primitive (grouped coloured polylines), not just a client-side filter. Independent of the per-vn eye; scope for the next P7 push. |
