...
| `cleanupImages.afCorrect` | `user-pick`. There is NO plan-time auto-signal: `afCombinations` lives only on the `afCorrect` task node (declared inside the chain), so the plan cannot read it. A per-set fluorophore panel would give one — §Q-M5 tracks the follow-up. Until then the user adds `afCorrect` to their chain by hand. | `user-pick` (default off, no auto-include) | **400** — after drift AND after smooth (Q-C6/C7 resolved 2026-09-06: smooth before AF, no composite) | Not excluded — the plan does not attempt to include it, so no exclusion reason applies. |
...
| `C-Galvo` | Galvo / clean signal | High-SNR, gaussian smoothing optional, AF via straightforward triangle background when the user has one to declare | W1 = `galvo` AND no prior `zeroFracIn ≥ 0.05` | `smooth` off; *`spatialMethod = gaussian`* if smooth is turned on later; `driftEstimator = multiLag`; `afCorrect` user-pick (no plan-time auto-signal — see §1 note); `denoise` user-pick |
...
- **§Q-M5 — Filter-set fingerprint** (1c §gap 5). This is the only path to a real plan-time
  auto-signal for `afCorrect`: a per-set (or per-project) fluorophore panel — declared once,
  cached on the set — would let (a) the plan auto-include `afCorrect` when a panel exists, and
  (b) the `afCorrect` task widget pre-populate `afCombinations` from that panel. Deferred
  2026-09-08 as a separate feature; until then `afCorrect` stays user-pick (§1) and the
  plan makes no attempt to include it.
