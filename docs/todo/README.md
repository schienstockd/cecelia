...
| [`CORRECTION_QC_PLAN.md`](CORRECTION_QC_PLAN.md) | **Phases A–E shipped + UI slice 3a (Q-M4 probes #811, §2.1 scores #813, §1/§3/§5 engine + cards #815, plan.json persistence #819, chain mount + card recommender #821, /api/correction-plan/{presets,recommend} + read-only CorrectionPlanPanel on CleanupModule this branch)** — the panel renders the auto-recommended plan (included steps + excluded reasons + QC scores) for one selected image; upstream_value_names stays deferred (chain executor owns wiring); Q-C1 + Q-C10 resolved by #810; **what's left:** UI slices 3b (card picker + save/load) / 3c (wizard W1–W6) / 3d (mount-to-chain), C-Deep3D card update once drift-vis-chain lands, remaining §Open questions | Metadata-driven decision tree + QC wizard that recommends (and can auto-run) a per-image `CorrectionPlan` — an ordered sequence of `cleanupImages.*` tasks — via named preset acquisition cards, computed QC scores, and a small "not derivable" wizard. Grounded in the three-part audit under `docs/archive/audit_phase1*.md`; every contradiction (C1–C10) is flagged for maintainer resolution, none silently picked. Uses the existing `ChainTemplate` as substrate, not a parallel executor |
