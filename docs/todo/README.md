...
| [`WEB_VIEWER_PLAN.md`](WEB_VIEWER_PLAN.md) | **P1-P4 built, P5 frame+sweep, P6+P7 built (P7.1 AF too)** (2026-08-28) · `feat/viewer-p71-af-preview` | Replacing napari with a browser WebGPU viewer (interactive) + the Julia compositor (offline movies), so cecelia becomes a sole browser app. The bar is PARITY with the bridge's 31 commands, not new capability — the checklist maps every one to a phase. 8 locked decisions, 8 phases (P1 the slab route + viewer, P2 the cached/playable timecourse, P3 the h5ad overlays — points/tails/colour-by, P4 masks), mask correction deferred. **Read the two sections at the end first**: the vertical-orientation fix that needs one click to confirm, and four written-down open questions. Every number comes from `NAPARI_WEBGPU_AUDIT.md`. Read before touching the viewer |
...
| [`KILN_BRICK_PLAN.md`](KILN_BRICK_PLAN.md) | **planning** (2026-08-28) · branch `feat/kiln-brick-port` | LOD-in-3D for the WebGPU viewer via a Kiln-style brick atlas + page-table indirection. The 3D path today loads the whole volume at one pyramid level (`pickVolumeLevel`); this plan replaces it with a physical brick atlas so near-camera regions render at L0/L1 while far bricks stay coarse, on a view-driven residency policy. Data source is the existing Julia slab endpoint (one axis-aligned slab per brick), not a new brick endpoint. 8 locked decisions incl. r8uint/r16uint atlas keyed on `bytesPerVoxel`, N-channel WGSL, T axis, 3D-halo prefetch; 8 phases P0-P7. Grounded in SispLk/35uedD (Manual IBEX, nZ=4, nC=25-38, uint8). Revises `WEB_VIEWER_PLAN.md` Decision 7 |
...
| [`VIEWER_CONTROLS_SPLIT_PLAN.md`](VIEWER_CONTROLS_SPLIT_PLAN.md) | **P0 done, P1-P9 planned** (2026-08-25) | Endgame: napari fully removed, WebGPU-only viewer. Napari today already does what Dominik wants (panel + pop manager feed it, napari doesn't pick its own); the WebGPU viewer grew its own selectors and needs them removed. Design: shared Pinia store as truth, panel writes / viewer + napari read. 9 phases, one per session: P0 template reshuffle (DONE), P1 shared image state (fixes "No image open in Napari" ghost), P2 lift panel-local state to store, P3-P5 delete viewer's selectors, P6 rename `napari*` → viewer-oriented, P7 native tracks, P8 native picking, P9 delete bridge |
