...
| [`CORRECTION_QC_PLAN.md`](CORRECTION_QC_PLAN.md) | **Phases A–D shipped (Q-M4 probes #811, §2.1 scores #813, §1/§3/§5 engine + cards #815, plan.json persistence + provenance this branch)**; §8 provenance reduced to (ceceliaVersion, saturationFingerprint) after review (writer_versions per step doesn't apply; upstream_value_names deferred to Phase E chain-mount); Q-C1 + Q-C10 resolved by #810; remaining §Open questions still maintainer-blocking | Metadata-driven decision tree + QC wizard that recommends (and can auto-run) a per-image `CorrectionPlan` — an ordered sequence of `cleanupImages.*` tasks — via named preset acquisition cards, computed QC scores, and a small "not derivable" wizard. Grounded in the three-part audit under `docs/archive/audit_phase1*.md`; every contradiction (C1–C10) is flagged for maintainer resolution, none silently picked. Uses the existing `ChainTemplate` as substrate, not a parallel executor |
