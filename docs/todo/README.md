...
| [`WEB_VIEWER_PLAN.md`](WEB_VIEWER_PLAN.md) | **P1 + P2 built** (2026-08-24) · `audit/napari-webgpu` | Replacing napari with a browser WebGPU viewer (interactive) + the Julia compositor (offline movies), so cecelia becomes a sole browser app. The bar is PARITY with the bridge's 31 commands, not new capability — the checklist maps every one to a phase. 8 locked decisions, 8 phases (P1 the slab route + viewer, P2 the cached/playable timecourse), mask correction deferred. Every number comes from `NAPARI_WEBGPU_AUDIT.md`. Read before touching the viewer |
