...
| [`CORRECTION_QC_PLAN.md`](CORRECTION_QC_PLAN.md) | **Phases A–E shipped (Q-M4 probes #811, §2.1 scores #813, §1/§3/§5 engine + cards #815, plan.json persistence #819, chain mount + card recommender this branch)** — a plan now yields a ChainTemplate the executor accepts and `recommend_card(scores, wizard)` picks a card so `recommend_plan` can auto-pick; upstream_value_names stays deferred (chain executor owns wiring); Q-C1 + Q-C10 resolved by #810; **what's left:** wizard + plan UI (frontend), C-Deep3D card update once drift-vis-chain lands, remaining §Open questions | Metadata-driven decision tree + QC wizard that recommends (and can auto-run) a per-image `CorrectionPlan` — an ordered sequence of `cleanupImages.*` tasks — via named preset acquisition cards, computed QC scores, and a small "not derivable" wizard. Grounded in the three-part audit under `docs/archive/audit_phase1*.md`; every contradiction (C1–C10) is flagged for maintainer resolution, none silently picked. Uses the existing `ChainTemplate` as substrate, not a parallel executor |
