...
| [`WEB_VIEWER_PLAN.md`](WEB_VIEWER_PLAN.md) | **planned** (2026-08-24) · no branch | Replacing napari with a browser WebGPU viewer (interactive) + the Julia compositor (offline movies), so cecelia becomes a sole browser app. Locked decisions, 7 phases, and the measurements from `NAPARI_WEBGPU_AUDIT.md` that each rests on. Read before touching the viewer |
