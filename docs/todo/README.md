...
| [`FLOW_REGISTER_ROBUSTNESS_PLAN.md`](FLOW_REGISTER_ROBUSTNESS_PLAN.md) | **planning** (2026-09-07) · branch `feat/flow-register-structural-channels` (plan only; phases on future branches) | Three additive follow-ups on `cleanupImages.flowRegister` derived from the Galene paper ([Warren et al. 2018, eLife 7:e35800](https://doi.org/10.7554/eLife.35800)) and its [frame-aligner](https://github.com/flimfit/frame-aligner) source: P1 per-frame post-warp correlation + `frameCorrelationFloor` gate (blank rather than pass warped garbage), P2 phase-correlation warm-start so Farneback only handles the residual after a cheap rigid pre-shift, P3 coverage-mask sidecar next to the OME-Zarr for downstream honesty. Decision 1: don't rewrite Farneback into Galene's control-point / LK optimiser — try the discipline around the estimator first. Anchored on c91ICQ (peak flow 115.82 px, 98% clamp saturation) |
