...
| [`CORRECTION_QC_PLAN.md`](CORRECTION_QC_PLAN.md) | **planning** (2026-09-06); no branch | Metadata-driven decision tree + QC wizard that recommends (and can auto-run) a per-image `CorrectionPlan` — an ordered sequence of `cleanupImages.*` tasks — via named preset acquisition cards, computed QC scores, and a small "not derivable" wizard. Design-only; grounded in the three-part audit under `docs/archive/audit_phase1*.md`; every contradiction (C1–C10) is flagged for maintainer resolution, none silently picked. Uses the existing `ChainTemplate` as substrate, not a parallel executor |
