...
| [`CORRECTION_PLAN.md`](CORRECTION_PLAN.md) | P1+P4a built (#590); P2 Merge/Remove + obs carry-over shipped; P3 invalidation surface shipped (staleness report tasks emit QC warn + sidecar); Phase-3 Review pager + Phase-4 `label.split` op shipped; raster brush (Draw/Erase/Fill/Pick) still open | Fix a wrong mask / wrong track in the browser correction cockpit and have the correction become the data everything downstream uses |
| [`CORRECTION_QC_PLAN.md`](CORRECTION_QC_PLAN.md) | **Phases A–E + UI slices 3a/3b/3c/3d + C-Deep3D card update shipped (Q-M4 probes #811, §2.1 scores #813, §1/§3/§5 engine + cards #815, plan.json persistence #819, chain mount + card recommender #821, /api/correction-plan/{presets,recommend} + read-only CorrectionPlanPanel #824, C-Deep3D `driftPerPlane`+`driftZSmoothness` per PR #818's compose finding #825, card picker + save/load plan.json #829, mount-to-chain #831, and wizard W2/W3/W5 (CollapsibleSection under the card row → three yes/no/unknown ChipSelects; W2 sitkRigid, W3 flowRegister, W5 stackAlign; W1 is the card picker; W4 = `exclusive: bool` already exists on `af_correct.json`, W6 = retired writer + switched to SUPPORT, both retired 2026-09-06 as unnecessary))** — UI surface is complete; upstream_value_names stays deferred (chain executor owns wiring); Q-C1 + Q-C10 resolved by #810; Q-C3, Q-C6/C7, Q-C4, Q-C8, Q-P3 resolved 2026-09-06 (drift-norm default `none`, smooth = bucket 300 before AF, AF ceiling dead as a cohort metric, default `meta.saturation` is the honest signal for denoise, cellposeCorrect retired); **what's left:** non-blocking / prompt-confirmation §Open questions only | Metadata-driven decision tree + QC wizard that recommends (and can auto-run) a per-image `CorrectionPlan` — an ordered sequence of `cleanupImages.*` tasks — via named preset acquisition cards, computed QC scores, and a small "not derivable" wizard. Grounded in the three-part audit under `docs/archive/audit_phase1*.md`; every contradiction (C1–C10) is flagged for maintainer resolution, none silently picked. Uses the existing `ChainTemplate` as substrate, not a parallel executor |
| [`CALL_FOR_DATASETS_PLAN.md`](CALL_FOR_DATASETS_PLAN.md) | **planning** (2026-09-04) | A discoverable in-app list of capabilities we can build once a real dataset lands, reachable from a new icon next to the GitHub-issues link in `AppHeader.vue`. Registry + `BaseModal` + prefilled GitHub-issue routing via `datasetAskUrl(ask)` (mirrors `recipeRequestUrl`). Seeded with two entries: full 6-DOF 3D rigid drift (`DRIFT_RIGID_PLAN` P5) and large static 3D registration. Deliberately narrow — Decision 7 forbids a status field, Decision 8 keeps entries in code |
| [`DRIFT_RIGID_PLAN.md`](DRIFT_RIGID_PLAN.md) | **planning → in-progress** (2026-09-04) · branch `audit/simpleitk-usage` · P1 – P4 built | Third `driftCorrect` estimator: rigid (`sitkRigid`) via SimpleITK `Euler2DTransform` (2D) / `Euler3DTransform` with X/Y rotations frozen (3D) for movies with in-plane stage rotation. Default stays `multiLag` — real datasets are translation-only. Full 6-DOF 3D rigid deferred to the Call-for-Datasets follow-up. 9 decisions + 5 phases: P1 `sitk_estimate_rigid` (built), P2 plumb through `estimate_drift` (built), P3 `apply_shifts` branch + JSON/QC, P4 vis aid (real algorithms, not hand-waves), P5 nice-to-haves. Grounded in [`../audit/simpleitk-opportunities.md`](../audit/simpleitk-opportunities.md) |
| [`FLOW_REGISTER_ROBUSTNESS_PLAN.md`](FLOW_REGISTER_ROBUSTNESS_PLAN.md) | **P1 metric shipped (PR #848), P2+P3 PARKED 2026-09-08** | Three additive follow-ups on `cleanupImages.flowRegister` derived from the Galene paper ([Warren et al. 2018, eLife 7:e35800](https://doi.org/10.7554/eLife.35800)) and its [frame-aligner](https://github.com/flimfit/frame-aligner) source. P1 per-frame post-warp `frameCorrelation` metric + `unalignedCorrelation` companion + cohort `meanFrameCorrelation` shipped metric-only. P2 (phase-correlation warm-start) + P3 (coverage-mask sidecar) PARKED after measuring on the anchor movie c91ICQ: the real failure fingerprint is 98% clamp saturation at `maxShiftPx=16` (peak flow 115.82 px), a magnitude problem no Galene phase addresses. Web-swept 2023-2026 successors (PatchWarp, RAFT/SEA-RAFT, VoxelMorph) — none target this fingerprint. **PatchWarp prototyped 2026-09-08 on both c91ICQ (3D) and EaMaVq (3P spleen); Farneback wins every frame on both movies (0/125, 0/200), worst PatchWarp frame -0.44 below Farneback. Ruled out.** Raising `maxShiftPx` to 128 also gave essentially unchanged correlation on both movies — the current 16px clamp is regularising and helping. Revisit only when a future movie's `frameCorrelation` shows a fingerprint distinct from both bulk motion AND per-pixel non-rigid deformation. |
...
| [`TRACK_SCHEME_PLAN.md`](TRACK_SCHEME_PLAN.md) | P1–P2 built + P2b (`feat/track-scheme`); **P6 shipped 2026-09-06** (PR #830); **P3 (untracked lane + points.add) shipped 2026-09-06** on `review/vizsla-track-editing` (click-based Add, not drag; first-label-only for multi-cell frames); **P4–P5 open** — P5 unblocked (viewer already picks segmentation labels via `pickCellAt`; P5 is the auto-subscription pass) | Timeline-first track correction: lanes over frames, so "can these two merge" is answered by the picture rather than by a greyed button. Records why the per-row and XY-only surfaces failed. **P5 + P6 draw on `napari-vizsla` (Tamas Nagy, MIT)** — the click-a-cell highlighting model + two-letter op hotkeys; CTC IO deliberately not adopted |
...
| [`CELL_CARDS_PLAN.md`](CELL_CARDS_PLAN.md) | planning (`feat/cell-cards`, 2026-09-08) | Per-cluster snapshot cards (medoid crop + track trace + stat footer) as a new `cellCards` InteractiveView on the Analysis board, `rail: 'clusterPops'`. "Card Table" = a layout preset, not a canvas. Grounded on fXgbTl's three trackclust pops |
...
| [`DESKTOP_ICON_PLAN.md`](DESKTOP_ICON_PLAN.md) | planning (2026-09-08) · branch `docs/feijoa-icon-plan` | Feijoa icon on the desktop-launcher entries `install.sh`/`install.ps1` write. Single SVG source (`frontend/public/feijoa.svg`); derived `.ico`/`.icns`/PNG committed under `frontend/public/icons/` so they ride the existing `frontend/dist` allow-list entry (release stays single-job ubuntu-latest, no `iconutil`). Linux is a **bug fix** — current `Icon=` points at the stale `favicon.svg`. Windows sets `$Shortcut.IconLocation`. macOS switches from bare `Cecelia.command` to a minimal `Cecelia.app` bundle. macOS validation runs on a **new `macos-latest` CI job** (public repo, runner cost is zero per `ci.yml:11`) that structurally checks the bundle and uploads a Finder `screencapture`; merge gated on green CI + eyeball on the screenshot. Gatekeeper first-launch UX is a one-off release-time human check. P1 (assets) → P2 Linux → P3 Windows → P4 macOS bundle + CI verify |
...
| [`DRIFT_JITTER_PLAN.md`](DRIFT_JITTER_PLAN.md) | **built** on `audit/drift-3d` — `driftSmoothSigma` param on `cleanupImages.driftCorrect`, task default σ=6 frames | Post-solve gaussian low-pass on the cumulative trajectory. Fixes the "jittering" the writer produced on movies whose estimated shifts sit at the PC noise floor (2h06xA: 107 → 11 integer-pixel transitions across 181 frames). Verified transparent on real motion (ttRMjQ: 168 px peak preserved to 3%). Smooths *positions* not deltas — per-delta thresholds amplify drift when noise partially cancels real spikes. Residuals still measured on the unsmoothed solution |
| [`DRIFT_3D_BREATHING_PLAN.md`](DRIFT_3D_BREATHING_PLAN.md) | **planning** on `audit/drift-3d` — research notes, no code | Within-stack (breathing during Z acquisition) and intra-frame (line-scan warp) correction, sitting BEFORE the existing rigid drift correction. Method survey (Fast4DReg, elastix groupwise, NoRMCorre, Greenberg-Kerr, probabilistic VoxelMorph) + prototyped block-wise PC + Farneback OF with fwd-bwd uncertainty on `anzjFk`. Design is a two-stage pipeline (clean b+c first, then existing driftCorrect); dep choice (CaImAn ~500 MB vs local ~50 line NoRMCorre port) still open |
| [`DRIFT_PERPLANE_PLAN.md`](DRIFT_PERPLANE_PLAN.md) | **in-progress** (2026-09-06) · branch `feat/drift-per-plane` — P1 built (multiLag/chain), no cross-Z coupling yet | Extends `estimate_drift` with `per_plane=True` — one 2D (Y, X) shift per (t, z) instead of one XYZ per t, for movies whose motion depends on Z depth (canonical case: breathing shear on `zolIMa/x4E5HU`, measured +4 px Y at shallow planes vs −9 px Y at deep planes between t40 and t41). Smallest useful thing in the [`DRIFT_3D_BREATHING_PLAN`](DRIFT_3D_BREATHING_PLAN.md) space — same rigid multiLag/chain machinery, per plane, no new deps. Same-YX canvas across Z, independent per-Z solves (Z-coupled prior deferred to P3). Sidecar gains `perPlane: bool` |
| [`STACK_ALIGN_PLAN.md`](STACK_ALIGN_PLAN.md) | **built** on `audit/drift-3d` — new task `cleanupImages.stackAlign` | Per-timepoint intra-stack XY alignment before `driftCorrect`. Whole-plane phase correlation against a chosen reference plane (`middle` or `sharpest`) with a confidence gate + max-shift clamp so structural Z differences don't get forced into shifts. Fixes issue #3 from the ttRMjQ audit (adjacent Z planes offset during acquisition); does NOT fix within-plane line-scan smear (issue #4, still QC-flag-only). Measured on `d5vw7z/c91ICQ`: median 0.9 px, max 7.8 px, ~40% edge planes correctly skipped by the gate |
