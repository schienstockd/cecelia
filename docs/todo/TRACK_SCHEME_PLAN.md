...
**Status:** planning · branch `feat/track-scheme`
...
- **P1 — the timeline, read-only.** Lanes, runs, candidate markers, sort + filter + window. No
  editing. Checkpoint: on the reference image (`zolIMa/1/fXgbTl`, 374 tracks) the gap candidates are
  visibly gaps and the overlapping pairs are visibly parallel.
- **P2 — editing from the timeline.** Select bars → Join (only when the runs do not overlap, and the
  overlap is visible when they do) · Split at the clicked frame · Remove. Plus "Fix", which queues a
  ticked candidate's own pre-picked op — the one-click path. Feeds the existing queue.
...
2. **How many lanes in the window** before it stops being legible — needs to be looked at, not
   guessed.
3. **Where it lives.** A third panel kind on the Track canvas beside `trackPaths`/`trackDiagnostics`,
   or does it *replace* `trackCorrection`? Replacing is the honest answer if P2 lands, but the
   worklist should not be deleted until the timeline can do everything it does.
