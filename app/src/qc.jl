...
# The QC copy catalog — every user-facing QC string, in one table. Extracted to its own file so
# the wording can be reviewed as a set without wading past the helpers. Rendering (`qc_text` and
# the code that reads QC files back) stays here.
include(joinpath(@__DIR__, "qc", "text.jl"))
...
function qc_text(key::AbstractString; subs...)
    haskey(QC_TEXT, key) || error("Unknown QC text key: \"$key\". Add it to QC_TEXT in app/src/qc/text.jl.")
    entry = QC_TEXT[key]
