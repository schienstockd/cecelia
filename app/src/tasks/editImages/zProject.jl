...
function _run_task(task::ZProject, img::CciaImage, params::Dict{String,Any};
    ...
    p          = parse_z_project_params(params)
    ccid       = state_file(img)
    ...
    proj_meta = Dict{String,Any}(
        ...
        "zproj_source_value_name" => p.valueName,
        "zproj_op"                => p.op)
    ...
    new_img = add_image!(s; name = "$(img.name) (z-$(p.op))", meta = proj_meta, attr = img.attr)

    out_filename = "ccidImage.ome.zarr"
    im_out_path  = joinpath(proj_dir, "0", new_img.uid, out_filename)
    on_log("[INFO] Z-project source: $im_path (op='$(p.op)')")
