...
import { INTERACTIVE_VIEWS, boardViews, pageViews, railFor, isPluginView, popTypesFor,
         VIEW_ALIASES, migrateViewKey } from './interactiveViews'
  ...
  it('every view a shipped example plugin names is offered to plugins', () => {
    ...
    for (const path of paths)
      for (const v of MANIFESTS[path].contributions?.views ?? []) {
        declared++
        // `pluginPage`, not merely registered: naming `trackScheme` would put a MUTATING view on
