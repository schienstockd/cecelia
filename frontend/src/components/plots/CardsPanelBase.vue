<!--
  Cards panel base — the shared spine for cellCards / motifCards / hmmCards.

  Owns: fetch + spinner + statScales + grid layout + PDF export + CanvasPanel wrap. Nothing here
  is family-specific: which endpoint to POST, what body to send, and what empty-state copy to
  show ALL come from a `CardFamily` config the caller supplies (see `cardFamilies.ts`).

  The family-specific detail panel (opened when a card is clicked) rides the default slot; the
  wrapper decides what to render for its family. `cellCards` renders `CellCardDetailPanel`.

  Design source: docs/todo/BEHAVIOUR_CARDS_PLAN.md — Decisions 1, 2 (family = config), 5 (shared
  render helper on the server), 8 (board-assets naming).
-->
...
async function fetchCards() {
  ...
  if (!rootUid.value || !props.suffix || !shownPops.length) return
  loading.value = true
  try {
...
// Grid tile shows the LAST frame of the filmstrip — the medoid's end.
...
<template>
  <CanvasPanel :index="index" :active="active" :arrange="arrange" :persist-key="persistKey"
    ...
    <div ref="hostRef" class="ccv" :class="{ 'cc-light': capturing }">
      ...
      <p v-else-if="!suffix" class="cc-muted">{{ family.emptyNoSuffix }}</p>
      <p v-else-if="!(shownPops && shownPops.length)" class="cc-muted">{{ family.emptyNoShownPops }}</p>
      ...
      <div v-else class="ccv-grid" :style="gridStyle">
        <div v-for="c in cards" :key="c.path" class="cc-card ccv-card">
          ...
          <div class="ccv-foot">
            <!-- Header row: entity name (never wraps — a two-line "Population 1" broke the export
      ...
      <!-- Family-specific detail panel goes here. Wrapper opens it in response to `cardSelect`. -->
...
/* frame wrapper carries the entity-colour ring; StripCell fills it edge-to-edge (no inset shadow) */
