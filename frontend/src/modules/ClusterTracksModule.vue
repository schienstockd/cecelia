<!--
  Cluster tracks module page. Pick one OR MORE images (multi-select), then:
   ...
   • below the table: explore clusters (heatmap + UMAP) and define populations by ticking cluster
     IDs into them (the `trackclust` pop type).

  Track counterpart of ClusterCellsModule.vue (one page per granularity, mirroring gate/track). Like it,
  this page only DEFINES populations — the `trackclust` population summary lives on **Behaviour**.
-->
