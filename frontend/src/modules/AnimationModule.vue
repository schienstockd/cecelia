<!--
  ...
  read whichever image napari happened to have open, so its empty state — "open an image in napari" —
  could only be acted on by leaving the page. The image table IS that action, and it brings the set bar,
  the filters and the eye with it. The controls moved into the side panel (animation/AnimationPanel.vue),
  the timeline into the standard plot canvas (animation/AnimationTimeline.vue).
...
<template>
  <ModuleLayout module="animation" :single-select="true" plots-label="Timeline" :right-default-width="290">
    ...
    <!-- With nothing selected the timeline still shows whatever napari has open, so landing on the page
         is never a blank canvas — that fallback is the helper, shared with the panel. -->
