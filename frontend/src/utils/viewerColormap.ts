// viewer colormap NAME → a representative hex "colour" for a legend swatch.
//
...
const NAPARI_COLORMAP_HEX: Record<string, string> = {
  ...
  // viewer `bop` single-hue colormaps (representative full-intensity colours)
 ...
 *  `value` is a valid viewer colormap name (passed straight to the viewer); the `hex` is derived from
...
/** Viewer colormap NAME for a hex (case-insensitive), or null if unknown. Exact match only — a
