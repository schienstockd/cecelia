...
export const ICON_LEGEND: IconFamily[] = [
  ...
  },
  {
    ...
    icons: [
      ...
      { icon: 'pi-user', means: 'Your active profile — click to open preferences' },
      { icon: 'pi-users', means: 'Show projects from every profile, not just yours' },
      { icon: 'pi-user-plus', means: 'Add a Kiwi profile (a separate credential + MCP scope)' },
      { icon: 'pi-user-minus', means: 'Retire a Kiwi profile — non-selectable after, data stays on disk' },
      { icon: 'pi-sign-in', means: 'Copy a login one-liner for this profile — paste in a terminal' },
  ...
  },
  {
    ...
    icons: [
      ...
      { icon: 'pi-arrow-right-arrow-left', means: 'Widen to the full range — an unclipped raw view' },
