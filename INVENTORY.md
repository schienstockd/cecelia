...
- **crop_render.jl**: in-app crop panel backend — reads OME-ZARR directly (`Zarr.jl`, sanctioned narrow carve-out — *previews only*, `zarr_utils` stays canonical) → coloured z-MIP PNG; `GET /api/crop/info` + `/api/crop/frame` (cached). Frontend: `CropPanel.vue` (scrubbable MIP + rectangle draw + z/t + Save → `editImages.cropImage`); pure box maths in `frontend/src/utils/crop3d.ts`. Replaced the napari 3D crop. See `docs/todo/CROP_PANEL_PLAN.md`.
