...
- **Project export/import**: `app/src/project_io.jl` — `export_project`/`import_project` (+ `default_export_dir`). Bundles a whole project to a `.ccbundle` (tree mirror + each `.zarr` store packed to one `.zarr.tar` via system `tar`, in parallel over `Threads.@spawn`) and restores it; background jobs (`jobs.jl`), no open project required. WS: `project:export`/`project:import` (`handle_project_export`/`handle_project_import` in `api/src/sockets.jl`); `list_bundles` + `GET /api/projects/bundles` feed the import picker. UI in `ProjectPanel.vue`; export destination + import source are chosen with the shared `FileBrowser.vue` (`mode="dir"`/`"bundle"` — browses any server path incl. mounts). Plan: `docs/todo/PROJECT_IO_PLAN.md`; jobs model: `docs/JOBS.md`.
