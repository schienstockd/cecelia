...
- **Gating engine**: `gating/population_manager.jl` (`PopulationMap`, `pop_df` unified accessor, `pop_df_multi` mixed-type module-function accessor — `resolve_pop_type`/`pop_namespace` — used by all spatial/cluster tasks so a picker can offer ANY pop type resolved to cells; `pop_name_conflict` cross-pop-type name-uniqueness guard used by the gating API pop add/rename; sidecar `gating/{vn}.json`), `gating/gating_engine.jl` (`recompute!`, membership, `cells_in_pop`), `gating/gates.jl`, `gating/transforms.jl` (logicle cited), `gating/density.jl`.
