...
- **Data patches (project-scoped maintenance)**: `app/src/maintenance.jl` — `MAINTENANCE_PATCHES` registry + `run_maintenance_patch`/`cancel_maintenance!`. Listed via `GET /api/maintenance/patches`; run via the `maintenance:run` WS message (`handle_maintenance_run`, streams over the task rail) confined to the open project; UI in SettingsModule "Data patches". NOT a `CciaTask` (whole-project, not image-scoped). Guide: `docs/DEV.md` → *Data patches*.
