...
function api_track_paths(req::HTTP.Request)
    ...
    # `occupancy=1` → timepoints only, for the track timeline: it draws lanes over FRAMES and reads
    # nothing but `t`. Its default cap is far higher because the two plots fail differently — a path
    # plot of 5000 polylines is an unreadable hairball, while a capped LANE LIST is a lie: "pick track
    # 2001" has no answer if 2001 was never sent, and the panel cannot even say it exists.
    occupancy = get(q, "occupancy", "") in ("1", "true")
    limit = something(tryparse(Int, get(q, "limit", "")), occupancy ? 20000 : 500)
