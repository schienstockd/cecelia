...
# subdirectories under `<config_dir()>/kiwi-profiles/`; retired ones stay in the list so a
# picker can show them grayed out and pass logs stay resolvable — the `retired` flag is what
# the UI keys off.
...
function api_kiwi_profiles_retire(body_bytes::Vector{UInt8})
    ...
    _kiwi_profile_retired(dir) &&
        return 200, JSON3.write((; ok = true, name = name, active = kiwi_profile_name(),
                                    alreadyRetired = true))
    write(_kiwi_retired_marker_path(dir), string(now()))
    snapped = false
    if kiwi_profile_name() == name
        set_kiwi_profile!("default")
        snapped = true
    end
    200, JSON3.write((; ok = true, name = name,
                        active = kiwi_profile_name(), snappedToDefault = snapped))
end
