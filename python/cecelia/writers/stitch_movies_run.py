...
grid cell); this composes them side-by-side (or stacked) via the shared helper
(``cecelia.utils.movie_io.stitch_movies``). Kept out of Julia because ``movie_io`` is the one
imageio writer in the repo, and captions come from ``title_card.caption_band`` which is the one
PIL text stack.
"""
