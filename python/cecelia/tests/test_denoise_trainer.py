...
class SupportTrainerSmokeTest(unittest.TestCase):
    ...
    def test_trainer_writes_pt_and_manifest_and_qc(self):
        ...
        # Early-stop signals — recorded on every training run so the QC handler can key off them
        # (`stoppedEarly=True` suppresses the "loss stayed flat" warn, since plateau is expected).
        # With epochs=1 and default patience, this run does NOT stop early — asserts the shape,
        # not the value.
        for k in ('stoppedEarly', 'stopEpoch', 'epochBudget'):
            self.assertIn(k, manifest['training'],
                          f'training.{k} missing — QC reads it from the manifest')
        self.assertEqual(manifest['training']['epochBudget'], 1)
        self.assertEqual(manifest['training']['stopEpoch'], 1)
        self.assertFalse(manifest['training']['stoppedEarly'])
        ...
        for k in ('finalLoss', 'firstLoss', 'lossDrop', 'epochLosses', 'epochs', 'nImages',
                  'stepLosses', 'stepIndices', 'stoppedEarly', 'stopEpoch'):
        ...
        self.assertFalse(qc['stoppedEarly'])   # 1-epoch run has no chance to plateau
