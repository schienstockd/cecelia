...
def run(params):
  ...
  task_root_dir = script_utils.get_param(params, 'taskRootDir')
  im_source_name = script_utils.get_param(params, 'imSourceName')
  labels_source_name = script_utils.get_param(params, 'labelsSourceName')
  label_props_value_name = script_utils.get_param(params, 'labelPropsValueName')
  im_task_dir = script_utils.get_param(params, 'imTaskDir')
  im_path = script_utils.get_param(params, 'imPath')
  im_labels_path = script_utils.get_param(params, 'imLabelsPath')
  im_label_props_path = script_utils.get_param(params, 'imLabelPropsPath')
  ...
  im_paths = {x: os.path.join(zero_root_dir, x, im_source_name) for x in uids}
  im_arrays = {i: zarr_utils.open_as_zarr(
    x, as_dask = True
    )[0][0] for i, x in im_paths.items()}
  
  # get label arrays
  im_labels_paths = {x: os.path.join(task_root_dir, x, 'labels', labels_source_name) for x in uids}
  im_labels_arrays = {i: zarr_utils.open_labels_as_zarr(x)[0][0] for i, x in im_labels_paths.items()}
  
  # get label props paths
  im_label_props_paths = {x: os.path.join(task_root_dir, x, 'labelProps', label_props_value_name) for x in uids}
  
  # get dim utils for images
  im_dim_utils = dict()
  for x in uids:
    # get OME-XML
    omexml = ome_xml_utils.parse_meta(im_paths[x])

    # set dimensions
    im_dim_utils[x] = DimUtils(omexml, True)
    im_dim_utils[x].calc_image_dimensions(im_arrays[x].shape)
  ...
  min_x = min([x.dim_val('X') for x in im_dim_utils.values()])
  min_y = min([x.dim_val('Y') for x in im_dim_utils.values()])
  ...
  im_combined = zarr.create(
    mode = 'w',
    shape = tuple(
        [im_dim_utils[uids[0]].dim_val('C')] + [
            num_rows * crop, num_cols * crop
        ]
    ),
    chunks = tuple((1, crop, crop)),
    dtype = im_arrays[uids[0]].dtype)
  
  im_labels_combined = zarr.create(
    mode = 'w',
    shape = im_combined.shape[1:],
    chunks = tuple((crop, crop)),
    dtype = np.uint32)
    
  # copy in images
  max_label = 0
  label_props_list = list()
  for i, x in im_arrays.items():
    ...
    row_i = math.floor(len(label_props_list)/num_cols)
    col_i = len(label_props_list) % num_cols
    ...
    # pop channels for label slices
    slices.pop(im_dim_utils[i].dim_idx('C'))
    labels = zarr_utils.fortify(im_labels_arrays[i][tuple(slices)]).astype(np.uint32)
    ...
    # get label props and append
    label_props_df = LabelPropsUtils(
      os.path.join(task_root_dir, i), value_name = label_props_value_name)\
        .label_props_view()\
        .filter_rows(np.unique(labels))\
        .as_df()
        
    # increase numbering
    label_props_df['label'] += max_label
    
    # copy slices
    im_combined[tuple(combined_slices)] = np.squeeze(im.astype(im_dtype))
    
    # drop channel for labels
    combined_slices.pop(0)
    labels[labels > 0] += max_label
    im_labels_combined[tuple(combined_slices)] = np.squeeze(labels)
    
    label_props_list.append(label_props_df)
  ...
  # measure labels and save back
  zarr_utils.create_multiscales(
    im_labels_combined, im_labels_path,
    x_idx = 2, y_idx = 1, keyword = 'labels'
    )
    
  # save label props back
  LabelPropsUtils(im_task_dir, value_name = 'default')\
    .label_props(
      pd.concat(label_props_list),
      save = True, split_columns = True
      )
