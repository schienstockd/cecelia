CreateCombinedImage <- R6::R6Class(
  ...
  public = list(
    ...
    run = function() {
      ...
      newLabelsFilepath <- "default.zarr"
      newLabelPropsFilepath <- "default.h5ad"
      
      attr(newFilepath, "partOf") <- uIDs
      attr(newLabelsFilepath, "partOf") <- uIDs
      attr(newLabelPropsFilepath, "partOf") <- uIDs
      
      newImage$setImFilepath(newFilepath)
      newImage$setImLabelsFilepath(newLabelsFilepath)
      newImage$setImLabelPropsFilepath(newLabelPropsFilepath)
      ...
      params <- list(
        ...
        labelsSourceName = basename(cciaObj$cciaObjects(uIDs = uIDs)[[1]]$imLabelsFilepath(
          valueName = "default"
        )),
        labelPropsValueName = "default",
        imTaskDir = newImage$persistentObjectDirectory(),
        imPath = newImage$imFilepath(),
        imLabelsPath = newImage$imLabelsFilepath(),
        imLabelsPropsPath = newImage$imLabelPropsFilepath(),
        uIDs = uIDs,
