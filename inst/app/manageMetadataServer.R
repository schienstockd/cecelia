    ...
    function(input, output, session) {
      ...
      observeEvent(input$selectAttribute, {
        ...
        if (input$selectAttribute != ""){
          shinyjs::enable("deleteAttribute")
          shinyjs::enable("singleValue")
          shinyjs::enable("regexpValue")
          shinyjs::enable("assignSingleValue")
          shinyjs::enable("assignRegexpValue")
          shinyjs::enable("assignGroupSequence")
        } else {
          shinyjs::disable("deleteAttribute")
          shinyjs::disable("singleValue")
          shinyjs::disable("regexpValue")
          shinyjs::disable("assignSingleValue")
          shinyjs::disable("assignRegexpValue")
          shinyjs::disable("assignGroupSequence")
        }
      })
      ...
      observeEvent(input$assignRegexpValue, {
        ...
        attrVals <- lapply(cciaObjects, function(x) {
          if (input$regexpSource == "oriFilepath")
            str_extract(x()$oriFilepath(), input$regexpValue)
          else
            str_extract(x()$getCciaName(), input$regexpValue)
        })
