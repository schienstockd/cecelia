  ...
  moduleServer(
    ...
    function(input, output, session) {
      ...
      observeEvent(input$setupHPCTest, {
        ...
        if (conOK == TRUE) {
          shinyjs::enable("hpcTask")
          shinyjs::enable("runHPCTask")
        }
      })
      ...
      observeEvent(input$runHPCTask, {
        ...
        if (!is.null(hpcTask)) {
          ...
          # clear output
          shinyjs::html(id = "hpcOutput", html = "*** START ***")
          shinyjs::disable("runHPCTask")
          shinyjs::enable("cancelHPCTask")
          ...
          o <- observe({
            ...
            if (hpcTask()$completed()) {
              ...
              # reset log
              hpcOutput("")
              shinyjs::html(id = "hpcOutput", html = paste0("<br/>", "*** END ***"), add = TRUE)
              
              # reset buttons
              shinyjs::enable("runHPCTask")
              shinyjs::disable("cancelHPCTask")
            } else {
      ...
      observeEvent(globalManagers$projectManager()$useHPC(), {
        if (globalManagers$projectManager()$useHPC() == TRUE) {
          shinyjs::enable("setupLabServerTest")
        } else {
          shinyjs::disable("setupLabServerTest")
        }
      })
