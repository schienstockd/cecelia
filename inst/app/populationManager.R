...
createPopulationManager <- function(
  ...
  observeEvent(input$colourChoice, {
    params = str_split(input$colourChoice, pattern = "\\.")[[1]]
    ...
    for (curColour in cciaConf()$colours$predefined) {
      ...
      if (colour == curColour) {
        shinyjs::html(labelID, htmlIcon(btnICON_SELECTED, btnClass = ""))
      } else {
        shinyjs::html(labelID, htmlIcon(btnICON_NOTSELECTED, btnClass = ""))
      }
  ...
  observeEvent(input$toggleVisibilityForPop, {
    ...
    if (popShow == TRUE) {
      ...
      shinyjs::html(labelID, htmlIcon(btnICON_SHOWN))
    } else {
      globalManagers$viewerManager()$viewer()$hideLayer(popLayerName)
      shinyjs::html(labelID, htmlIcon(btnICON_HIDDEN))
    }
  })
