    ...
    function(input, output, session) {
      ...
      resetObjInfoFromGatingSet <- function(
        ...
        for (x in pops) {
          ...
          if (!cciaObj()$flowGatingSet()$popIsRoot(popParent)) {
            if (!is.na(str_match(x, popParent)[[1]])) {
      ...
      flowShapeToGateCoords <- function(shapeObj) {
        ...
        if ("path" %in% names(shapeObj) && !is.na(shapeObj$path)) {
          # create freeform
          pathCoords <- str_split(shapeObj$path, "L")[[1]]
          
          # What does 'M' and 'Z' mean?
          pathCoords <- str_replace(pathCoords, "M", "")
          pathCoords <- str_replace(pathCoords, "Z", "")
          
          # convert to numbers
          pathCoords <- str_split(pathCoords, ",")
      ...
      flowGatingPlotsShapeEdits <- eventReactive(c(
        ...
        for (i in names(retVal)) {
          if (any(is.na(str_match(names(retVal[[i]]), "shapes\\[[0-9]+\\]")))) {
            retVal[[i]] <- NULL
          }
        }
      ...
      observeEvent(flowGatingPlotsShapeEdits(), {
        ...
        gateID <- as.numeric(str_extract(names(shapeObj), "[0-9]+")[[1]]) + 1
        
        # prepare shape object
        names(shapeObj) <- str_replace(
