...
createImageSetManager <- function(
  ...
  filteredUIDs <- reactive({
    ...
    if (length(applyFilters) > 0) {
      ...
      for (i in names(applyFilters)) {
        ...
        filterCol <- str_replace(i, "attrFilter_[:alnum:]+_", "")
  ...
  observeEvent(imageSets(), {
    ...
    if (length(imageSets()) > 0) {
      shinyjs::enable("selectSet")
      shinyjs::enable("reloadSet")
    } else {
      shinyjs::disable("selectSet")
      shinyjs::disable("reloadSet")
    }
  })
  ...
  observeEvent(imageSetFilters(), {
    ...
    if (any(lengths(imageSetFilters()) > 0)) {
      shinyjs::enable("attrFilterApply")
      shinyjs::enable("attrFilterReset")
      shinyjs::enable("attrFilterInvert")
    } else {
      shinyjs::disable("attrFilterApply")
      shinyjs::disable("attrFilterReset")
      shinyjs::disable("attrFilterInvert")
    }
  })
