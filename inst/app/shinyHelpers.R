  ...
  for (i in seq(startFrom, length(postfix))) {
    if (!is.null(initDisabled) &&
      ...
      inputs[i] <- as.character(
        shinyjs::disabled(createInput(id, i, postfix[i]))
      )
    } else {
...
trimInputName <- function(sessionID, inputID) {
  str_split(inputID, paste0(sessionID, "-"))[[1]][[2]]
}
...
getShinyBookmarkStatePath <- function(stateID, bookmarkStateDir = NULL) {
  ...
  curFile <- curDirs[!is.na(str_match(curDirs, stateID))]
...
toggleButtonBoolean <- function(
  ...
  # toggle classes
  shinyjs::removeClass(id, classFALSE)
  shinyjs::addClass(id, classTRUE)
  
  # change button labels
  if (!is.null(btnLabels)) {
    if (checkRes == TRUE) {
      shinyjs::html(id, btnLabels$success)
    } else {
      shinyjs::html(id, btnLabels$fail)
    }
  }
}
