...
class LabelPropsUtils(LabelPropsView):
  ...
  """
  def label_props(self, label_props, save = False, obs_cols = ['label'], update_existing = False,
                  obsm = dict(), uns = dict(), split_columns = False):
    # split off columns
    if split_columns is True:
      # get spatial and temporal columns from props
      centroid_spatial = [x for x in label_props.columns if x in [f'centroid_{i}' for i in ['x', 'y', 'z']]]
      ...
      if len(centroid_spatial) > 0:
        ...
        obsm['spatial'] = label_props[centroid_spatial].to_numpy()
        
      if len(centroid_temporal) > 0:
        uns['temporal_cols'] = centroid_temporal
        obsm['temporal'] = label_props[centroid_temporal].to_numpy()
            
    # get positive and negative location positions
    obs_loc = [x in obs_cols for x in label_props.columns]
    other_loc = [x not in obs_cols for x in label_props.columns]
    ...
    if update_existing is True:
      ...
      self.adata = super().create_new_dataset(
        label_props.loc[:, other_loc].to_numpy().astype(np.float32),
          var = pd.DataFrame(index = label_props.columns[other_loc]),
          obs = self.adata.obs
      )
    else:
      self.adata = super().create_new_dataset(
