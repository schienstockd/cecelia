...
# anaDir <- "/Volumes/USER_data/Dominik/Experiments/JL-ANALYSIS/RESULTS/XCR1-Venus/densities"
anaDir <- "/Volumes/USER_data/Dominik/Experiments/JL-ANALYSIS/RESULTS/XCR1-Venus/tracks"
```
...
funParams <- list(
  ...
  # pops = c("tcells.gDT/tracked"),
  # colName = "movement",
  # hmmStates = c("movement"),
  colName = "tonly_movement",
  hmmStates = c("tonly_movement"),
  # colName = "shape",
  # hmmStates = c("shape"),
  includeStart = FALSE,
...
funParams <- list(
  ...
  valueName = "tracks.clusters.tonly.hmm_transitions",
  clusterColName = "tonly_hmm_transitions",
  ...
  # resolution = 0.30, # all types
  resolution = 0.25, # T only
  percentile = 99.8,
  trackMeasures = c(
    "speed",
    "duration",
  ...
  objectMeasures = c(
    ...
    "live.cell.hmm.state.tonly_movement",
    "live.cell.hmm.transitions.tonly_movement"
...
exp.info <- as.data.table(cciaObj$summary(withSelf = FALSE, fields = c("Attr", "oriFilepath")))
uIDs <- exp.info$uID
...
types.sc <- as.data.table(cciaEnv()$LabelPropsUtils(
  ...
  value_name = "tracks.clusters.tonly.hmm_transitions.sc")$label_props_view()$as_df())
...
```{r}
# classify CD8+ tracks and show track measures
paramsToGet <- c("live.cell.is.clust", "live.cell.speed")

tracksInfo <- rbindlist(list(
  gBT = cciaObj$tracksInfo(paramsToGet, parentPop = "tcells.gBT/tracked", uIDs = uIDs)
), idcol = "pop") %>% left_join(
  cciaObj$tracksMeasures(pops = c("tcells.gBT/tracked"), uIDs = uIDs)
)
```

```{r fig_track_measures, fig.height=4, fig.width=12}
# set whether track is clustering
tracksInfo[, track.is.clust := FALSE]
tracksInfo[live.cell.is.clust.TRUE >= 0.6, track.is.clust := TRUE]

colsToPlot <- c(
  "speed", "duration", "trackLength", "meanTurningAngle", "displacement", 
  "straightness", "displacementRatio", "outreachRatio", "asphericity", "overallAngle"
)

datToPlot <- tracksInfo %>%
  pivot_longer(cols = colsToPlot, names_to = "measure", values_to = "value")

# now plot out common measures
ggplot(datToPlot, aes(track.is.clust, value)) +
...
ggsave(file.path(anaDir, "track_measures.png"), height = 4, width = 12)
write.csv(datToPlot, file.path(anaDir, "track_measures.csv"))
```
...
# map clusters
types.sc <- .mapClustNames(types.sc, clusterMapping)
# popDTs <- .mapClustNames(popDTs, clusterMapping, "live.cell.track.clusters.hmm_transitions")
popDTs <- .mapClustNames(popDTs, clusterMapping, "live.cell.track.clusters.tonly_hmm_transitions", removeNone = FALSE)
```


## Density measurements

```{r fig_niches, fig.height=8, fig.width=8}
# show regions
p1s <- list()
...
colPal <- list(
  ...
  `Meandering prolate` = "#B3BCC2",
  Meandering = "#B3BCC2",
  Scanning = "#4682b4"
)

for (i in uIDs) {
  for (j in c("tcells.gBT/tracked", "tcells.gDT/tracked")) {
    for (k in c("tcells.gBT/tracked", "dcs.all/tracked")) {
      p1 <- ggplot(popDTs[uID == i & pop == j] %>% drop_na(clusters.name),
                   aes(centroid_x, -centroid_y, color = as.factor(clusters.name))) +
        geom_density2d(data = popDTs[uID == i & pop == k], size = 0.4, color = "dimgray") +
        geom_point(size = 0.4) +
        theme_classic() +
        coord_fixed() +
        # scale_color_brewer(name = NULL, palette = "Set1") +
      ...
      ggsave(file.path(anaDir, "niches", paste0(
        "niches_", i, "_", .flowPopParent(j), "_to_", .flowPopParent(k), ".png")),
        height = 5, width = 8, plot = p1)
    }
  }
}
```
...
# for (i in c("9sKC8P")) {
  ...
  x.gBT <- popDTs[uID == local_i & pop == "tcells.gBT/tracked"]
  x.gDT <- popDTs[uID == local_i & pop == "tcells.gDT/tracked"]
  x.DCs <- popDTs[uID == local_i & pop == "dcs.all/tracked"]
  
  dens <- MASS::kde2d(
    x.DCs$centroid_x, x.DCs$centroid_y, n = n,
    lims = c(
      range(x.DCs$centroid_x) * c(1 - extendLimits, 1 + extendLimits),
      range(x.DCs$centroid_y) * c(1 - extendLimits, 1 + extendLimits)
      ))
  # dens <- MASS::kde2d(
  ...
  # push back
  popDTs[uID == local_i & pop == "tcells.gBT/tracked", in_contour := x.gBT$in_contour]
  popDTs[uID == local_i & pop == "tcells.gDT/tracked", in_contour := x.gDT$in_contour]
  # popDTs[uID == local_i & pop == "tcells.gDT/tracked", in_contour_gBT := x.gDT$in_contour]
}
```
...
# ggplot(x.gBT, aes(centroid_x, -centroid_y, color = as.factor(live.cell.hmm.state.tonly_movement))) +
ggplot(x.gDT, aes(centroid_x, -centroid_y, color = as.factor(live.cell.hmm.state.tonly_movement))) +
  geom_polygon(
    data = confidenceDT,
    aes(x = x, y = -y, group = as.factor(-seq)),
    size = 0.2, color = "black", fill = "white") +
  geom_path(aes(group = track_id)) +
  geom_point(size = 0.5) +
  theme_classic() +
  coord_fixed() +
...
```{r fig_dens_behaviour, fig.height=5, fig.width=11}
# can you show the transitions based on distance to XCR1
# basically show the frequency of transitions based on distance
# this would be on gDT to XCR1 distance

# can you plot out the behaviour sections according to DC density?
datToPlot <- as.data.table(popDTs[pop %in% c("tcells.gBT/tracked", "tcells.gDT/tracked")] %>%
  drop_na(live.cell.hmm.transitions.tonly_movement) %>%
  mutate(
    group = stringr::str_extract(live.cell.hmm.transitions.tonly_movement, "[0-9]+(?=_)"),
    transition = stringr::str_extract(live.cell.hmm.transitions.tonly_movement, "(?<=_)[0-9]+"),
    split = in_contour > 0.5
  ) %>%
  dplyr::filter(
    live.cell.hmm.transitions.tonly_movement != "NA",
    # exclude self
    # group != transition
    ) %>%
  group_by(uID, pop, group, transition, in_contour) %>%
  # group_by(uID, pop, split, group, transition) %>%
  # group_by(uID, pop, split, group, transition) %>%
  # group_by(uID, pop, in_contour, live.cell.hmm.transitions.tonly_movement) %>%
  # drop_na(live.cell.hmm.state.tonly_movement) %>%
  # group_by(uID, pop, live.cell.hmm.state.tonly_movement, in_contour) %>%
  summarise(n = n()) %>%
  mutate(freq = n/sum(n)) %>%
  ungroup() %>%
  complete(uID, pop, group, transition, in_contour, fill = list(freq = 0)) %>%
  # complete(uID, pop, split, group, transition, fill = list(freq = 0)) %>%
  # complete(uID, pop, in_contour, live.cell.hmm.transitions.tonly_movement, fill = list(freq = 0)) %>%
  # complete(uID, pop, live.cell.hmm.state.tonly_movement, in_contour, fill = list(freq = 0)) %>%
  left_join(exp.info))

# colPal <- randomcoloR::distinctColorPalette(length(unique(datToPlot$live.cell.hmm.transitions.tonly_movement)))
# colPal <- randomcoloR::distinctColorPalette(length(unique(datToPlot$live.cell.hmm.state.tonly_movement)))

# ggplot(datToPlot, aes(in_contour, freq, color = clusters.name, fill = clusters.name)) +
# ggplot(datToPlot[datToPlot$uID == "ZmxDIj",] %>% dplyr::filter(live.cell.hmm.transitions.tonly_movement != "NA"), aes(
# ggplot(datToPlot %>% dplyr::filter(live.cell.hmm.transitions.tonly_movement != "NA"), aes(
ggplot(datToPlot[pop == "tcells.gBT/tracked"], aes(
  # in_contour, freq, color = as.factor(live.cell.hmm.transitions.tonly_movement),
  # fill = as.factor(live.cell.hmm.transitions.tonly_movement))) +
  in_contour, freq, color = as.factor(transition), fill = as.factor(transition))) +
  # split, freq, color = as.factor(transition))) +
  # in_contour, freq, color = as.factor(live.cell.hmm.state.tonly_movement),
  # fill = as.factor(live.cell.hmm.state.tonly_movement))) +
  theme_classic() +
  ...
  # geom_jitter(
  #   position = position_jitterdodge(jitter.width = 0.10), alpha = 1.0) +
  facet_grid(transition~group)
  # facet_grid(.~group)
  # facet_grid(.~pop)
  # facet_grid(pop~as.factor(live.cell.hmm.transitions.tonly_movement))

ggsave(file.path(anaDir, "transitions_to_DC_dens.png"), height = 4, width = 8)
```

```{r fig_dens_behaviour, fig.height=4, fig.width=12}
# can you plot out the behaviour sections according to DC density?
datToPlot <- popDTs[pop %in% c("tcells.gBT/tracked", "tcells.gDT/tracked")] %>%
# datToPlot <- popDTs[pop %in% c("tcells.gDT/tracked")] %>%
  # drop_na(clusters.name) %>%
  drop_na(live.cell.hmm.state.tonly_movement) %>%
  # group_by(uID, pop, in_contour, clusters.name) %>%
  group_by(uID, pop, in_contour, live.cell.hmm.state.tonly_movement) %>%
  # group_by(uID, pop, in_contour_gBT, clusters.name) %>%
  summarise(n = n()) %>%
  mutate(freq = n/sum(n)) %>%
  ungroup() %>%
  # complete(uID, pop, in_contour, clusters.name, fill = list(freq = 0)) %>%
  complete(uID, pop, in_contour, live.cell.hmm.state.tonly_movement, fill = list(freq = 0)) %>%
  # complete(uID, pop, in_contour_gBT, clusters.name, fill = list(freq = 0)) %>%
  left_join(exp.info)

# ggplot(datToPlot, aes(in_contour, freq, color = clusters.name, fill = clusters.name)) +
ggplot(datToPlot, aes(in_contour, freq,
                      color = as.factor(live.cell.hmm.state.tonly_movement),
                      fill = as.factor(live.cell.hmm.state.tonly_movement))) +
# ggplot(datToPlot, aes(in_contour_gBT, freq, color = clusters.name, fill = clusters.name)) +
  theme_classic() +
  geom_smooth() +
  scale_color_brewer(name = NULL, palette = "Set1") +
  scale_fill_brewer(name = NULL, palette = "Set1") + 
  # facet_grid(.~pop)
  facet_wrap(.~pop+uID, ncol = 8)

# ggsave(file.path(anaDir, "behaviour_to_DC_dens.png"), height = 2, width = 6)
# ggsave(file.path(anaDir, "behaviour_to_DC_dens_indv.png"), height = 8, width = 16)
# ggsave(file.path(anaDir, "behaviour_to_DC_dens_HMM.png"), height = 2, width = 6)
ggsave(file.path(anaDir, "behaviour_to_DC_dens_indv_HMM.png"), height = 8, width = 16)
```
...
ggsave(file.path(anaDir, "HMM_heat_tonly.pdf"), width = 2.5, height = 3)
```
...
ggsave(file.path(anaDir, "umap_tonly.png"), width = 4, height = 4)
...
ggsave(file.path(anaDir, "pop_clusters_freq_tonly.png"), width = 5, height = 1.2)
```
...
ggsave(file.path(anaDir, "clusters_heat_tonly.pdf"), width = 5.5, height = 6)
