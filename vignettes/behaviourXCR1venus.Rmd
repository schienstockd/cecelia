...
ggplot(datToPlot, aes(UMAP_1, UMAP_2)) +
  ...
  # geom_point(aes(color = volume.mean), size = 1, alpha = 1) +
  # geom_point(aes(color = sphericity.mean), size = 1, alpha = 1) +
  # geom_point(aes(color = ellipticity_interm_oblate.mean), size = 1, alpha = 1) +
  # geom_point(aes(color = ellipticity_interm_prolate.mean), size = 1, alpha = 1) +
  geom_point(aes(color = compactness.mean), size = 1, alpha = 1) +
...
# ggsave(file.path(anaDir, paste0("umap_Volume.tiff")), width = 2, height = 2)
# ggsave(file.path(anaDir, paste0("umap_Sphericity.tiff")), width = 2, height = 2)
# ggsave(file.path(anaDir, paste0("umap_Oblate.tiff")), width = 2, height = 2)
# ggsave(file.path(anaDir, paste0("umap_Prolate.tiff")), width = 2, height = 2)
ggsave(file.path(anaDir, paste0("umap_Compact.tiff")), width = 2, height = 2)
```
...
for (i in unique(datToPlot$pop)) {
  ...
  plot.p1(datToPlot, datToPlot %>% dplyr::filter(pop == i))
  ggsave(file.path(anaDir, paste0("umap_HSV_", x, ".tiff")), width = 2, height = 2)
}
```
...
ggplot(
  dfToPlot %>%
    mutate(pop = factor(
      ...
      ),levels = c("Dendritic cells", "gDT-II", "gBT-I"))) %>%
    group_by(pop, clusters) %>%
    summarise(n = n()) %>%
    mutate(freq = n/sum(n),),
  aes(x = pop, y = freq, fill = clusters)) +
...
ggsave(file.path(anaDir, "pop_clusters_freq.tiff"), width = 4, height = 1.5)
```
...
ggsave(file.path(anaDir, "cluster_pop_freq.tiff"), width = 6, height = 2)
```
...
ggsave(file.path(anaDir, "clusters_hmm_heat.pdf"), width = 4, height = 1.5)
```
...
datToPlot <- popDTs %>%
  mutate(
    live.cell.track.clusters.hmm_transitions = as.numeric(as.character(
      live.cell.track.clusters.hmm_transitions)),
  ...
  # group_by(live.cell.track.clusters.hmm_transitions, from, to) %>%
  ...
  # ungroup() %>%
  # complete(live.cell.track.clusters.hmm_transitions, from, to, fill = list(freq = 0))
...
ggplot(datToPlot, aes(as.factor(from), as.factor(to), size = freq * 2, fill = freq)) +
  ...
  facet_grid(live.cell.track.clusters.hmm_transitions~hmm_type)

ggsave(file.path(anaDir, "clusters_transitions_heat.pdf"), width = 4, height = 8)
```
