...
funParams <- list(
  ...
  # channelDivisor = "gBT.CTDR",
  channelDivisor = "OTI-CTV",
  normOrder = 5,
  peakThreshold = 1.5,
  minPeaks = 2,
...
# summaryDF <- summaryDF[, .(n.clusters = .N), by = .(uID, pop, clusters.name)] %>%
  ...
  left_join(exp.info)
summaryDF$Treatment.DTx.pop <- as.character(paste(summaryDF$Treatment, summaryDF$DTx, summaryDF$pop))
...
ggplot(summaryDF,
       aes(clusters, freq.clusters, color = as.factor(Treatment.DTx.pop))) +
