...
branchingDT <- as.data.table(read.csv(file.path(anaDir, 'stroma', 'n22foC_branching.csv')))
```

```{r}
branchingDT[, c("path.id", "coord.src.1", "coord.src.2"), `:=` c("coord.src.1" = "x", "coord.src.2" = "y")]
```


```{r}
rbind(
  branchingDT[, c("path.id", "coord.src.1", "coord.src.2"), `:=` c("coord.src.1" = "x", "coord.src.1")],
  branchingDT[, c("path.id", "coord.dst.1", "coord.dst.2")]
)
```
...
branchingDT[rbind(
  branchingDT[, c("path.id", "coord.src.1", "coord.src.2")],
  branchingDT[, c("path.id", "coord.dst.1", "coord.dst.2")]
), on = "path_id"]
```


```{r fig_branching, fig.height=5, fig.width=8}
# show branching of stroma network
# ggplot(branchingDT, aes(coord.dst.2, coord.dst.1, color = branch.distance)) +
# ggplot(branchingDT, aes(coord.dst.2, coord.dst.1, color = euclidean.distance)) +
