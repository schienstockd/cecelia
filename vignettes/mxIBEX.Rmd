...
```{r}
a <- readRDS("/Volumes/Analysis_SSD/Dominik/cecelia/projects/wEsnzt/ANALYSIS/1/n22foC/ccid.rds")
```
...
# ggsave(file.path(anaDir, 'stroma', 'n22foC_branching_zoom.png'), plot = p1, height = 20, width = 20)
...
popDT <- cciaObj$popDT(
  "clust", includeFiltered = TRUE, completeDT = TRUE,
  popCols = c("pop", "clusters"), pops = c("root"), forceReload = TRUE)
...
summaryDT <- copy(popDT)
summaryDT[branch.dist > 20, `branch-type` := -1]

summaryDT <- summaryDT %>%
  # drop_na() %>%
  # group_by(pop, regionsNN) %>%
  group_by(clusters, `branch-type`) %>%
  summarise(n = n()) %>%
...
p1 <- ggplot(summaryDT, aes(reorder(clusters, freq), freq, fill = `branch-type`)) +
  theme_classic() +
  ylim(0, 100) +
  geom_col() +
...
ggsave(file.path(anaDir, 'stroma', 'n22foC_branching_type_clusters.png'), plot = p1, height = 4, width = 4)
```
