...
devtools::load_all("../")
cciaUse("~/Desktop/DOHERTY/cecelia", initConda = TRUE)
...
```{r fig_comp, fig.height=2.5, fig.width=3}
...
# init ccia object
cciaObj <- initCciaObject(
  pID = pID, uID = "JU2zQg", versionID = versionID, initReactivity = FALSE
)
cciaSet <- initCciaObject(
  pID = pID, uID = "rcjHiw", versionID = versionID, initReactivity = FALSE
)

gs <- cciaObj$flowGatingSet()$getPopObj()
...
oriDT <- .flowFortifyGs(gs, cols = c(xLabel, yLabel), subset = "comp")
comDT <- .flowFortifyGs(
  .flowCompensateGs(flowWorkspace::gs_clone(gs), cciaSet$flowAutospillMatrix()),
  cols = c(xLabel, yLabel), subset = "comp")
gsDT <- rbindlist(list(ori = oriDT, comp = comDT), idcol = "type")

ggplot(gsDT, aes(x = get(xLabel), y = get(yLabel), color = type)) +
  theme_classic() +
