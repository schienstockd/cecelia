...
```{r fig_angles, fig.height=3, fig.width=3}
# now plot out
datToPlot <- coordMappings %>%
  group_by(uID) %>%
  summarise(mean = mean(angle, na.rm = TRUE)) %>%
  left_join(exp.info)

# ggplot(datToPlot, aes(Region, mean)) +
ggplot(coordMappings %>% left_join(exp.info), aes(Region, angle)) +
  theme_classic() +
  # geom_boxplot(outlier.alpha = 0) +
  geom_violin(scale = "width") +
  geom_jitter(width = 0.2) +
  # geom_jitter(position = position_jitterdodge(jitter.width = 0.10)) +
  stat_summary(fun=mean, geom="point", size=8, shape=18, color="#AA1F5E") +
  ylim(0, 100)
...
# ggplot(datToPlot, aes(Region, mean)) +
ggplot(coordMappings %>% left_join(exp.info), aes(Region, aniso)) +
  theme_classic() +
  # geom_boxplot(outlier.alpha = 0) +
  geom_violin(scale = "width") +
  geom_jitter(width = 0.2) +
