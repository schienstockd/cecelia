...
# init ccia object
cciaObj <- initCciaObject(
  pID = pID, uID = "v7oSIV", versionID = versionID, initReactivity = FALSE
)
...
boxAnisotropy <- abs(uns$fibres$kwLJ2A$ilee_eigval[1,,,1] - uns$fibres$kwLJ2A$ilee_eigval[1,,,2])
totalLength <- sum(uns$fibres$kwLJ2A$ilee_box_total_length)
anisotropy <- sum(boxAnisotropy) / totalLength
```
...
for (x in unique(tracksInfo$uID)) {
  # can you calculate the anisotropy of local track/fibre?
  boxAnisotropy <- abs(uns$fibres[[x]]$ilee_eigval[1,,,1] - uns$fibres[[x]]$ilee_eigval[1,,,2])
  localLength <- uns$fibres[[x]]$ilee_box_total_length
