...
for (x in unique(bootTracksInfo$uID)) {
  print(paste(">>", x))
  
  y <- cciaObj$cciaObjectByUID(x)[[1]]
  ...
  for (bootID in seq(bootIterations)) {
    ...
    coordMapping_rot <- (bootTracksInfo[uID == x] %>%
      left_join(coords$fibres[[x]]$coords %>% tibble::rownames_to_column(),
                by = c("centroid_y_tile" = "y", "centroid_x_tile" = "x"))) %>%
      left_join(
        # popDT[uID == x] %>%
    ...
    coordMappingsList[[paste(x, bootID, sep = "-")]] <- copy(coordMapping)
  }
}

coordMappingsBoot <- rbindlist(coordMappingsList)
```
...
coordMappingsTotal <- rbindlist(list(coordMappings, coordMappingsBoot), use.names = TRUE, fill = TRUE)
```
