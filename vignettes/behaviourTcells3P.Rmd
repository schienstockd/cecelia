...
cciaObj <- initCciaObject(
  pID = pID, uID = "gkS7Qn", versionID = versionID, initReactivity = FALSE # SHG 3P
)

tracks <- list(B6 = cciaObj$tracks("tcells.B6"))
...
# plot quiver
coords <- data.frame(
  y = as.vector(uns$ilee_coor_list[1,,,1]),
  x = as.vector(uns$ilee_coor_list[1,,,2])
)
u <- as.vector(uns$ilee_eigval[1,,,2]) * as.vector(uns$ilee_eigvec[1,,,2,1])
v <- as.vector(uns$ilee_eigval[1,,,1]) * as.vector(uns$ilee_eigvec[1,,,1,1])
```
...
```{r fig_tracks_grid, fig.height=10, fig.width=10}
# get average delta and velocity and plot .. ?
tracksGrid <- popDT %>%
  group_by(pop, centroid_region) %>%
  summarise(
    x = median(centroid_x, na.rm = TRUE),
    y = median(centroid_y, na.rm = TRUE),
    velocity = median(centroid_dist, na.rm = TRUE),
    u = log(velocity) * median(delta_centroid_x, na.rm = TRUE),
    v = log(velocity) * median(delta_centroid_y, na.rm = TRUE),
    # u = velocity * median(delta_centroid_x, na.rm = TRUE),
    # v = velocity * median(delta_centroid_y, na.rm = TRUE),
  )

ggplot(tracksGrid) + 
  theme_classic() +
  # geom_path(
  #   data = popDT[value_name == "tcells.OTI"],
  #   aes(centroid_x, centroid_y, group = track_id), color = "#327EBA", size = 1.5) +
  # geom_path(
  #   data = popDT[value_name == "tcells.gBT"],
  #   aes(centroid_x, centroid_y, group = track_id), color = "#327EBA", size = 1.5) +
  # geom_path(
  #   data = popDT[value_name == "tcells.P14"],
  #   aes(centroid_x, centroid_y, group = track_id), color = "#327EBA", size = 1.5) +
  ggquiver::geom_quiver(aes(x = x, y = y, u = -u, v = v, color = pop), linewidth = 2) +
  theme_classic() +
  plotThemeDark() +
  coord_fixed() +
...
p1 + geom_segment(aes(
  ...
  ggquiver::geom_quiver(
    data = coords, aes(x = x, y = y, u = -u, v = v), color = "grey", size = 2) 
  # geom_path(
  #   data = popDT, aes(centroid_x, centroid_y, group = track_id),
  #   color = "#ebd441", size = 1.5)
```
...
tracksMeasuresDT <- popDT[tracksInfo, on = c("track_id")]

p1s <- list()
for (i in tracksMeasures) {
  local({
    local_i <- i
    p1s[[local_i]] <<- p1 + geom_path(
    # p1s[[local_i]] <<- p1 + geom_bin_2d(
      data = tracksMeasuresDT, 
      aes(centroid_x, centroid_y, group = track_id, color = get(local_i)),
