...
ggsave(file.path(anaDir, "clusters", "indv", paste0("aSf5Rr_tumour.png")), height = 10, width = 13)
```
...
popNN <- dbscan::kNN(
  as.matrix(branchingDT[`branch-type` > 0, c('centroid_y', 'centroid_x')]),
  # as.matrix(branchingDT[, c('centroid_y', 'centroid_x')]),
  k = 1,
  query = as.matrix(popDT[, c('centroid_y', 'centroid_x')]))

# assign dists to pops
popDT[, stroma.dist := popNN$dist]
```
