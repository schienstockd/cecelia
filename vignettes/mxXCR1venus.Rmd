...
pops <- cciaObj$popPaths(popType = "flow", uIDs = uIDs[2], includeFiltered = TRUE)
...
pops <- c("/nonDebris/P14")

popDT <- cciaObj$popDT(
  popType = "flow", uIDs = uIDs[2], pops = pops, includeFiltered = FALSE)

popPPP <- cciaObj$ppp(
  popType = "flow", pops = pops, uIDs = uIDs[2], usePhysicalScale = TRUE,
  hullType = "concave", concavity = 2, splitWindowBy = "flow.cell.clust.id")
```
