...
p1s <- .flowPlotGatedRaster(
  cciaObj, popPaths = "root", labelSize = 5, asContours = TRUE)
...
# ggsave(file.path(anaDir, "TK5dex_gating.pdf"), width = 8, height = 8)
```
...
pops <- c("/nonDebris", "/nonDebris/P14", "/nonDebris/XCR1")
popsWindow <- c("/nonDebris")
popsPP1 <- c("/nonDebris/P14")
popsPP2 <- c("/nonDebris/XCR1")
...
```{r fig_spatial_stats, fig.height = 1.5, fig.width = 1.5}
# plot out clark evans
dataToPlot <- exp.info %>%
  drop_na(pp1.clarkevans.R) %>%
  pivot_longer(
    # cols = c("pp1.clarkevans.R", "pp2.clarkevans.R"),
    cols = c("pp1.clarkevans.R"),
    names_to = "spatial",
    values_to = "value"
  )

ggplot(dataToPlot, aes(
  spatial, value,
  fill = Treatment)) +
  theme_classic() +
  geom_boxplot(outlier.alpha = 0) +
...
summaryDT[pop.to %in% c(
  # "/nonDebris/O/B20", "/nonDebris/O/O1/CD3", "/nonDebris"
  # "/nonDebris/O/B220", "/nonDebris"
), ]$pop.to <- "Other"
summaryDT[pop.to %in% c("/nonDebris/O/O1/CD3"), ]$pop.to <- "T"
...
summaryDT[pop.to == "/nonDebris",]$pop.to <- "Other"
summaryDT[pop.to == "/nonDebris/O/B220",]$pop.to <- "B"
summaryDT[pop.to == "/nonDebris/XCR1",]$pop.to <- "XCR1"
```
