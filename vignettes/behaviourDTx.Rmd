...
# spatialDT.sc <- copy(spatialDT)
spatialDT.sc[
  popDTs[, c("uID", "label", "track_id")],
  on = c("uID", "to" = "label"),
  track_id.to := track_id]
spatialDT.sc[
  tcells.sc[, c("uID", "track_id", "clusters")],
...
```{r fig_region_freq, fig.height=5, fig.width=5}
# show regions
p1 <- ggplot(popDTs.niches.gBT %>% left_join(exp.info), aes(centroid_x, centroid_y, color = as.factor(niche.cluster))) +
  geom_point(size = 0.5) +
  theme_classic() +
  coord_fixed() +
  scale_color_brewer(name = NULL, palette = "Set1") +
