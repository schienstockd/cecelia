...
ggsave(file.path(anaDir, "HMM_heat.pdf"), width = 4, height = 3)
```
...
```{r fig_umap, fig.height=8, fig.width=8}
datToPlot <- copy(tcells.sc) %>% drop_na(clusters) %>% left_join(exp.info)
...
ggplot(
  ...
  theme_classic() +
  geom_point(color = "#B3BCC2", size = 2) +
  geom_point(data = datToPlot %>% dplyr::filter(Treatment != "HSV"),
...
ggsave(file.path(anaDir, paste0("umap_Uninf.tiff")), width = 5, height = 5)
...
ggplot(
  ...
  theme_classic() +
  geom_point(color = "#B3BCC2", size = 2) +
  geom_point(data = datToPlot %>% dplyr::filter(Treatment == "HSV", DTx == "PBS"),
             color = "#AA1F5E", size = 2, alpha = 1) +
  theme(
    axis.text = element_text(size = 15),
    axis.text.x = element_blank(),
...
ggsave(file.path(anaDir, paste0("umap_HSV_PBS.tiff")), width = 5, height = 5)
...
ggsave(file.path(anaDir, paste0("umap_HSV_DTx.tiff")), width = 5, height = 5)
```
...
ggsave(file.path(anaDir, "clusters_heat.pdf"), width = 10, height = 12)
```
...
summaryDF$clusters.name <- "NONE"
summaryDF[summaryDF$clusters %in% c(1,7), ]$clusters.name <- "Directed"
summaryDF[summaryDF$clusters %in% c(3), ]$clusters.name <- "Clustering"
summaryDF[summaryDF$clusters %in% c(2,5,6), ]$clusters.name <- "Meandering"
summaryDF[summaryDF$clusters %in% c(0,4), ]$clusters.name <- "Scanning"

# show frequency of clusters
summaryDF <- summaryDF[, .(n.clusters = .N), by = .(uID, pop, clusters)] %>%
# summaryDF <- summaryDF[!clusters %in% c(1,8), .(n.clusters = .N), by = .(uID, pop, clusters)] %>%
