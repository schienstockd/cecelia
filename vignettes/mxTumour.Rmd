...
cciaObj <- initCciaObject(
  pID = pID, uID = "mmEf5V", versionID = versionID, initReactivity = FALSE
)
...
popDT <- cciaObj$popDT("clust", includeFiltered = TRUE)
```

```{r fig_tumour_border, fig.height=3, fig.width=12}
ggplot(popDT[pop == "MHCII+"], aes(centroid_x, -centroid_y, color = `clust.cell.contact#branch.CD31.branch__two`)) +
  geom_point() +
  theme_classic() +
  coord_fixed() 
...
# init ccia object
cciaObj <- initCciaObject(
  pID = pID, uID = "Ynvgzy", versionID = versionID, initReactivity = FALSE
)

exp.info <- as.data.table(cciaObj$summary(withSelf = FALSE, fields = c("Attr")))
uIDs <- c("Ad8GPQ")

popPaths <- cciaObj$popPaths(popType = "flow", uIDs = uIDs)

# can you extract the tumour border as a line?
popDT <- cciaObj$popDT(popType = "flow", uIDs = uIDs, pops = popPaths)
```
