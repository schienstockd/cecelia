...
baseChannels <- c(
  'CD164','MUC-1','p53','Vimentin','Cytokeratin',
  'PD-L1','CD15','CD30','CD2','GranzymeB',
  'CD5','MMP-9','CD4','LAG-3','CD25','CD56',
  'CD20','PD-1','CD11c','CD162','CD16',
  'CD11b','CD194','IDO-1','EGFR','VISTA',
...
funParams <- list(
  ...
  clusterChannels = list(
    base = list(channels = unname(sapply(baseChannels, function(x) which(imChannels == x))) - 1),
    nuc = list(channels = unname(sapply(nucChannels, function(x) which(imChannels == x))) - 1)
  ),
...
markers <- list(
  init = c(
    baseChannels, paste0("nuc_", nucChannels), objectMeasures
  )
)
...
# get pops
pops <- cciaObj$popPaths(uIDs = uIDs, popType = "clust", includeFiltered = TRUE)
...
summaryDF <- popDT %>%
  # dplyr::filter(clusters %in% c(5,11)) %>%
