...
for (x in cciaObj$cciaObjects()) {
  x$setOriFilepath(paste0("/Volumes/Lab-Mueller/Dominik Schienstock/Notebook/Experiments/CODEX_HCC/", x$getCciaName(), "/reg001_cyc002_ch001_HOECHST 2.tif"))
  x$saveState()
}
```
...
clusterChannels <- clusterChannels[!clusterChannels %in% c(
  ...
  "CD56", "vGLUT1", "PSD-95", "FoxP3", "CD20", "CD44", "Synaptophysin", "CD68"
  # NEXT
...
# for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  ...
  # ggsave(file.path(anaDir, "UMAPs", paste0("umap-", x$getUID(), ".png")), width = 3, height = 3, plot = p1)
...
# p1 <- ggplot(popDT,
p1 <- ggplot(popDT[!clusters %in% seq(0, 7)],
# p1 <- ggplot(dfToPlot,
...
# for (i in c("SiaqvM")) { 
  ...
  # p1 <- ggplot(regionsDT[uID == i & !regions %in% c(6,2)],
               aes(centroid_x, -centroid_y, color = as.factor(clusters))) +
