...
# anaDir <- "/Volumes/MDHS-Research/5300/5260/Lab-Mueller/Dominik Schienstock/Notebook/Experiments/CODEX_HCC/ANALYSIS"
```
...
cciaObj <- initCciaObject(
  pID = pID, uID = "hR7gDX", versionID = versionID, initReactivity = FALSE 
)
...
cciaObj <- initCciaObject(
  ...
  pID = pID, uID = "DPKHD7", versionID = versionID, initReactivity = FALSE # Set
)
...
# init ccia object
cciaObj <- initCciaObject(
  pID = pID, uID = "oUPuBX", versionID = versionID, initReactivity = FALSE # Human HCC
)
...
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  ...
  imChannelNames[imChannelNames == "pancyto"] <- "pancytokeratin"
  imChannelNames[imChannelNames == "TCRValpha"] <- "TCRVa"
  imChannelNames[imChannelNames == "FOXP3"] <- "FoxP3"
  imChannelNames[imChannelNames == "CD4 good"] <- "CD4"
  imChannelNames[imChannelNames == "ILR18a"] <- "IL18Ra"
  x$setImChannelNames(imChannelNames)
  x$saveState()
}
```
...
# init ccia object
cciaObj <- initCciaObject(
  # pID = pID, uID = "jGcZHp", versionID = versionID, initReactivity = FALSE 
  # pID = pID, uID = "oUPuBX", versionID = versionID, initReactivity = FALSE 
  pID = pID, uID = "DPKHD7", versionID = versionID, initReactivity = FALSE
)

# uIDs <- exp.info[Include == "Y"]$uID
# uIDs <- uIDs[!uIDs %in% c("AtIzds")]
uIDs <- names(cciaObj$cciaObjects())
...
# init ccia object
cciaObj <- initCciaObject(
  pID = pID, uID = "jGcZHp", versionID = versionID, initReactivity = FALSE # THEO MOUSE
  # pID = pID, uID = "akAcC1", versionID = versionID, initReactivity = FALSE # Set
  # pID = pID, uID = "pN5QJi", versionID = versionID, initReactivity = FALSE # Human LN
  # pID = pID, uID = "oUPuBX", versionID = versionID, initReactivity = FALSE # Human HCC
  # pID = pID, uID = "DPKHD7", versionID = versionID, initReactivity = FALSE # THEO Human
)
...
uIDs <- names(cciaObj$cciaObjects())
# uIDs <- exp.info[Include == "Y"]$uID
uIDs <- exp.info[Analyse == "Y"]$uID # THEO MOUSE
# uIDs <- exp.info[Stain == "A"]$uID
...
channelNames <- lapply(cciaObj$cciaObjects(uIDs = uIDs), function(x) unname(x$imChannelNames()))
...
clusterChannels <- clusterChannels[!clusterChannels %in% c("DAPI", "DAPI-02", "HOECHST")]
...
clusterChannels <- clusterChannels[!clusterChannels %in% c(
  ...
  "CD20", "CD11c", "LamB1", "CD8", "CD206/MRC1", "CD45",
  "Tryptase", "NF", "TMEM119", "LamB1", "SOX2"
  # NEXT
...
funParams <- list(
  ...
  popType = "flow",
  popsToCluster = c(),
  # popsToCluster = c("Zero"),
  savePops = TRUE,
  # keepPops = TRUE,
  keepPops = FALSE,
  # resolution = 0.2, # THEO MOUSE BRAIN, initial
  # resolution = 0.4, # Human LN
  # resolution = 0.6, # Human HCC
  resolution = 0.8, # THEO MOUSE BRAIN, 2nd
  mergeUmap = TRUE,
  normalisePercentile = 99.8,
  normalisePercentileBottom = 0.0,
  normaliseAxis = "channels",
  normaliseToMedian = FALSE, # Human LN and HCC
  # normaliseToMedian = TRUE, # MOUSE THEO
...
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  ...
  # get populations to do clustering on
  # THEO MOUSE
  funParams$popsToCluster <- unlist(unname(x$popPaths("flow")))
...
cciaObj <- initCciaObject(
  ...
  # pID = pID, uID = "akAcC1", versionID = versionID, initReactivity = FALSE # Set
  # pID = pID, uID = "pN5QJi", versionID = versionID, initReactivity = FALSE # Set
  # pID = pID, uID = "oUPuBX", versionID = versionID, initReactivity = FALSE # Set
  pID = pID, uID = "DPKHD7", versionID = versionID, initReactivity = FALSE # THEO Human
)
...
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  ...
  # x$setImNeighboursFilepath("default.sq.h5ad")
  # x$setImRegionsFilepath("default.region.h5ad")
  x$saveState()
}
```

```{r fig_umap_clustered, fig.height=6, fig.width=6}
# run clustering
devtools::load_all("../")
cciaUse("~/cecelia/dev", initConda = FALSE)
...
task <- cciaObj$runTask(
  funName = "clustPopulations.integrateClustering",
  funParams = funParams,
  env = "local",
  runInplace = TRUE,
  taskID = 1
...
# check UMAP
umapDF <- readRDS(file.path(cciaObj$persistentObjectDirectory(), "data", "default.cluster.groups.rds"))
colPal <- randomcoloR::distinctColorPalette(length(unique(umapDF$clusters.group)))

# umapDF <- .mapClustNames(umapDF, clusterMapping, "clusters.group", removeNone = FALSE)

p1 <- ggplot(umapDF, aes(V1, V2)) +
  # geom_point(aes(color = uID), size = 2) +
  geom_point(aes(color = clusters.group), size = 2) +
  # geom_point(aes(color = clusters.name), size = 3) +
  theme_classic() +
  # scale_color_brewer(name = NULL, palette = "Set1") +
  scale_color_manual(values = colPal) +
  # scale_color_manual(name = NULL, values = clusterColours) +
...
ggsave(file.path(anaDir, "group_umap.png"), width = 4, height = 5, plot = p1)
```
...
```{r}
x <- cciaObj$cciaObjectsByUIDs(c("VRZml0"))[[1]]
...
# init ccia object
cciaObj <- initCciaObject(
  # pID = pID, uID = "jGcZHp", versionID = versionID, initReactivity = FALSE # TEHO Mouse
  # pID = pID, uID = "akAcC1", versionID = versionID, initReactivity = FALSE # Set
  pID = pID, uID = "pN5QJi", versionID = versionID, initReactivity = FALSE # Human LN
  # pID = pID, uID = "oUPuBX", versionID = versionID, initReactivity = FALSE # Human HCC
  # pID = pID, uID = "DPKHD7", versionID = versionID, initReactivity = FALSE # THEO Human
...
# regionPops <- cciaObj$popPaths(uIDs = uIDs[[1]], popType = "region", includeFiltered = TRUE)
...
data.table::fwrite(summaryDF, file = file.path(anaDir, "freq_regions.csv"))
```
...
spe <- cciaObj$spe(
  popType = "clust", pops = pops, uIDs = uIDs, includeFiltered = TRUE, completeDT = TRUE)
```

```{r fig_spiat_pops, fig.height=6, fig.width=10}
x <- cciaObj$cciaObjects(uIDs = uIDs)[[1]]
imPopMap <- x$imPopMap(popType = "clust", includeFiltered = TRUE)
...
SPIAT::plot_cell_categories(
  spe[[1]], phenotypes, unname(popColours), "Phenotype", cex = 0.1) +
  coord_fixed()

ggsave(file.path(anaDir, "SPIAT", paste0(names(spe)[[1]], "_map.png")), height = 4, width = 6)
```

```{r}
gradient_pos <- seq(50, 500, 50) # radii

spiat.entropies <- lapply(spe, function(x) {
...
```{r fig_spe_entity, fig.height=10, fig.width=12}
for (i in unique(entropiesDT$uID)) {
  datToPlot <- entropiesDT[uID == i] %>%
    pivot_longer(
...
```{r fig_regions, fig.height=20, fig.width=30}
# init ccia object
# cciaObj <- initCciaObject(
#   pID = "Fu1zEC", uID = "pN5QJi", versionID = 1, initReactivity = FALSE # LN Set
# )
# 
# # get experimental info
# exp.info <- as.data.table(cciaObj$summary(withSelf = FALSE, fields = c("Attr")))
...
# 
# popDT <- cciaObj$popDT(
#   popType = "clust", uIDs = uIDs, includeFiltered = TRUE, completeDT = TRUE)

# get colours
colPal2 <- randomcoloR::distinctColorPalette(length(unique(popDT$clusters)))

# for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
#   # plot UMAP
#   p1 <- ggplot(popDT[uID == x$getUID()], aes(UMAP_1, UMAP_2)) +
...
p1 <- ggplot(popDT, aes(UMAP_1, UMAP_2)) +
  geom_point(aes(color = clusters), size = 0.5) +
  theme_classic() +
  scale_color_manual(name = "Clusters", values = colPal2) +
  guides(colour = guide_legend(override.aes = list(size=5))) +
...
ggsave(file.path(anaDir, "umaps.tiff"), width = 6, height = 4, plot = p1)
```
...
# could you cluster clusters across samples?
# because I have no idea what most of these markers are ...
# get matrix
...
clusters <- dbscan::dbscan(
  umapDF[, c("V1", "V2")], eps = 2, minPts = 4, borderPoints = TRUE)

umapDF$clusters.group <- as.factor(clusters$cluster)
# umapDF <- .mapClustNames(umapDF, clusterMapping, "clusters.group", removeNone = FALSE)

colNames <- unique(umapDF$clusters.group)
# colPal <- randomcoloR::distinctColorPalette(length(unique(umapDF$uID)))
colPal <- randomcoloR::distinctColorPalette(length(unique(umapDF$clusters.group)))
# colPal <- randomcoloR::distinctColorPalette(length(unique(colNames)))
# colPal <- randomcoloR::distinctColorPalette(length(colNames))
names(colPal) <- colNames

# save to set
# saveRDS(umapDF, file = file.path(cciaObj$persistentObjectDirectory(), "data", "cluster.groups.rds"))
# umapDF <- readRDS(file.path(cciaObj$persistentObjectDirectory(), "data", "cluster.groups.rds"))
...
# }
# 
# popDT[, clusters.group := as.factor(clusters.group)]
...
popDT <- .mapClustNames(popDT, clusterMapping, "clusters.group", removeNone = FALSE)
```
...
channelsToShow <- list(
  "DAPI" = "bop blue",
  "GFAP" = "gray",
  # "CD3" = "magenta", # THEO Mouse
  "CD3e" = "magenta", # THEO Human
  "Iba-1" = "green" # THEO Mouse
  )
popsToShow <- list(
  # flow = list(
...
generateScreenshots(
  ...
  list(), fileAttrs, size = NULL, save = TRUE)
...
colNames <- unique(popDT$clusters)
# colNames <- unique(umapDF$clusters.group)
colPal <- randomcoloR::distinctColorPalette(length(colNames))
names(colPal) <- colNames
...
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  ...
  # dfToPlot <- popDT %>% drop_na(clusters) %>% left_join(exp.info)
  # dfToPlot <- popDT[!clusters %in% seq(0, 7)] %>% drop_na(clusters) %>% left_join(exp.info)
  # dfToPlot <- .mapClustNames(dfToPlot, clusterMapping, "clusters")
  ...
  dfToPlot$clusters <- factor(dfToPlot$clusters)
  # dfToPlot$clusters.group <- factor(dfToPlot$clusters.group)
  ...
  p1 <- ggplot(
    ...
    geom_point(aes(color = clusters)) +
    # geom_point(aes(color = clusters.group)) +
  ...
  # ggsave(file.path(anaDir, "UMAPs", paste0("umap-", cciaObj$getUID(), ".png")), width = 4, height = 4, plot = p1)
  ggsave(file.path(anaDir, "UMAPs", paste0("umap-", exp.info[uID == x$getUID()]$Treatment, "-", x$getUID(), ".png")), width = 4, height = 5, plot = p1)
  # ggsave(file.path(anaDir, "UMAPs", paste0("umap-", x$getUID(), ".png")), width = 4, height = 4, plot = p1)
}
...
for (i in unique(popDT$uID)) {
  ...
  # go through clusters
  # for (j in unique(x$clusters)) {
  for (j in unique(x$pop)) {
    print(paste(">", j))
    ...
    p1 <- ggplot(x,
      ...
      # geom_point(size = 0.2) +
      geom_point(data = x, size = 0.2, color = "darkgrey") +
      # geom_point(data = x[clusters == j], size = 0.2, color = "red") +
      geom_point(data = x[pop == j], size = 0.2, color = "red") +
      # scale_color_brewer(palette = "Set3", name = "Clusters") +
    ...
    # ggsave(file.path(anaDir, "maps", paste0("clusters-", exp.info[uID == i]$Treatment, "-", i, "-", j, ".png")), width = 5, height = 5, plot = p1)
    ggsave(file.path(anaDir, "maps", paste0("pop-", exp.info[uID == i]$Treatment, "-", i, "-", j, ".png")), width = 5, height = 5, plot = p1)
  }
}
```
...
# print out population definitions
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  popDef <- sapply(
    x$imPopMap("clust", includeFiltered = TRUE),
    function(y) if (length(y$filterValues) > 0)paste(y$path, paste(y$filterValues, collapse = ","), sep = ": "))
  
  popDefs[[x$getUID()]] <- as.data.table(t(as.data.frame(popDef[lengths(popDef) > 0])))
}

data.table::fwrite(rbindlist(popDefs, idcol = "uID"), file.path(anaDir, "population_definitions.csv"))
```


```{r fig_clusters_freq, fig.height=5, fig.width=15}
...
levelOrder <- unique(exp.info[uID %in% uIDs]$Treatment)
summaryDF$Treatment <- factor(summaryDF$Treatment, levels = levelOrder)

colPal <- as.list(randomcoloR::distinctColorPalette(length(unique(summaryDF$pop))))

# plot
ggplot(summaryDF,
       # aes(paste(Treatment, uID), freq, fill = clusters)) +
       aes(paste(Treatment, uID), freq, fill = pop)) +
  theme_classic() +
  geom_col() +
  coord_flip() +
  # scale_fill_manual(values = colPal, name = "Clusters") +
  # xlab("") + ylab("Clusters (%)")
  scale_fill_manual(values = colPal, name = "Pops") +
  xlab("") + ylab("Population (%)")

# ggsave(file.path(anaDir, "clusters-freq.png"), width = 8, height = 4)
ggsave(file.path(anaDir, "pops-freq.png"), width = 8, height = 5)
```
...
x <- cciaObj$cciaObjects(uIDs = uIDs[[1]])[[1]]
xres <- x$omeXMLPixelRes()$x

# # imPopMap <- x$imPopMap(popType = "clust", includeFiltered = TRUE)
imPopMap <- x$imPopMap(popType = "region", includeFiltered = TRUE)
popColours <- sapply(imPopMap, function(x) {x$colour})
names(popColours) <- sapply(imPopMap, function(x) {x$path})

# channelCols <- unname(x$imChannelNames(correctChannelNames = TRUE))
# channelCols <- channelCols[channelCols %in% colnames(popDT)]
# channelCols <- channelCols[!channelCols %in% c("HOECHST")]
# dfToPlot <- as.data.table(popDT %>%
#   pivot_longer(cols = channelCols, names_to = "channels", values_to = "intensity"))

# clusters.groups <- popDT %>% group_by(uID, clusters.group) %>% summarise() %>% group_map(~.x)
# clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))), 10) # HCC
# clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))), 4, 11, 5, 9) # THEO MOUSE
# clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))))

popClusters <- unique(popDT$clusters)
# popClusters <- unique(popDT$clusters.group)
# popClusters <- unique(popDT$clusters.name)
# popClusters <- unique(regionsDT$regions)
# popClusters <- clusters.groups
# colPal <- as.list(randomcoloR::distinctColorPalette(length(popClusters)))
# colPal <- list("#4169e1", "#ff1493", "#ffd700", "#00fa9a", "#ff8c00", "#d8bfd8", "#e9967a", "#ff0000")
# colPal <- list("#ff0000", "#ff8c00", "#ffd700", "#0000cd", "#00ff00", "#00fa9a",
#                "#4169e1", "#e9967a", "#00ffff", "#ff1493", "#d8bfd8", "#ff00ff",
#                "#f0e68c", "#ee82ee", "#00bfff")
names(colPal) <- popClusters

# for (i in unique(dfToPlot$uID)) {
for (i in unique(regionsDT$uID)) {
# for (i in unique(popDT$uID)) {
# for (i in c("XFHvVd")) {
# for (i in c("SiaqvM")) { 
  # x <- popDT[uID == i & clusters.name == "CD68.CD11c"] %>%
  #                left_join(spatialDT[pop.from == "CD68.CD11c" & pop.to == "CD3"], by = c("uID", "label" = "from"))
  # z <- x[is.na(pop.to)]
  # y <- x[!is.na(pop.to)]
  # 
  # if (nrow(x) == 0) next
  ...
  # p1 <- ggplot(spatialDT[uID == i & pop.from == "CD3" & pop.to == "CD11c"] %>%
  # p1 <- ggplot(x,
  # p1 <- ggplot(popDT[uID == i],
  # p1 <- ggplot(popDT[uID == i & clusters.group %in% clusters.groups],
  # p1 <- ggplot(popDT[uID == i &
  # p1 <- ggplot(popDT[uID == i & !clusters %in% seq(0,7) &
                       # centroid_y >= 1849/xres & centroid_y <= 7196/xres &
                       # centroid_x >= 3535/xres & centroid_x <= 7920/xres],
                       # centroid_y >= 2096/xres & centroid_y <= 4406/xres &
                       # centroid_x >= 1200/xres & centroid_x <= 4597/xres],
  # p1 <- ggplot(popDT[uID == i & !clusters.group %in% c(3, 12, 5, 13)],
  p1 <- ggplot(regionsDT[uID == i],
               # aes(centroid_x, -centroid_y, color = as.factor(clusters))) +
               # aes(centroid_x, -centroid_y, color = as.factor(regions))) +
               # aes(centroid_x, -centroid_y, color = GFAP)) +
               aes(centroid_x, -centroid_y, color = as.factor(pop))) +
               # aes(centroid_x, -centroid_y, color = as.factor(clusters.group))) +
               # aes(centroid_x, -centroid_y, color = as.factor(clusters.name))) +
               # aes(centroid_x, -centroid_y)) +
    theme_classic() +
    plotThemeDark(angle = 0) +
    ...
    geom_line(data = data.frame(centroid_x = c(10, 510),  centroid_y = c(10, 10)), color = "white", size = 5) +
    # geom_point(aes(color = intensity), size = 0.1, alpha = 1) +
    # geom_point(data = z, color = .flowColours(z$centroid_x, z$centroid_y, colramp = viridisLite::inferno)) +
    # geom_point(data = y, color = .flowColours(y$centroid_x, y$centroid_y, colramp = viridisLite::inferno)) +
    # scale_color_brewer(palette = "Accent", name = "Clusters") +
    # scale_color_brewer(palette = "Set1", name = "Clusters") +
    # scale_color_manual(values = colPal, name = "Clusters") +
    # scale_color_manual(values = colPal, name = "Regions") +
    # scale_color_manual(values = clusterColours, name = "pop") +
    scale_color_manual(values = popColours, name = "Pop") +
    # scale_color_manual(values = c("NA" = "grey", "CD3" = "red"), name = "Pop") +
    ...
    # facet_wrap(.~clusters.group, ncol = 6)
    # facet_wrap(.~clusters.name, ncol = 6)
    # facet_wrap(.~regions, ncol = 6)
    # facet_grid(.~pop.to)
    # facet_wrap(.~channels, ncol = 8)
    # facet_grid(clusters~uID)
    # facet_grid(clusters.name~uID)
    # ggtitle(i)
  
  # ggsave(file.path(anaDir, "maps", paste0("clusters_", i, ".png")), width = 6, height = 8, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("clusters_group_", i, ".png")), width = 6, height = 8, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("clusters_name_", i, ".png")), width = 12, height = 16, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("labels_", i, ".png")), width = 30, height = 20, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("clusters_", i, "-dist.png")), width = 8, height = 5, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("clusters_", i, "-zoom.png")), width = 12, height = 16, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("pops_", i, ".png")), width = 12, height = 12, plot = p1)
  ggsave(file.path(anaDir, "maps", paste0("regions_", i, ".png")), width = 12, height = 12, plot = p1)
  # ggsave(file.path(anaDir, "maps", paste0("clusters_facet_", i, ".png")), width = 20, height = 20, plot = p1)
}
```

```{r}
# plot all images out with the same populations
x <- cciaObj$cciaObjects(uIDs = uIDs[[1]])[[1]]
xres <- x$omeXMLPixelRes()$x

clusters.groups <- popDT %>% group_by(uID, clusters.group) %>% summarise() %>% group_map(~.x)
# clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))), 10) # HCC
clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))), 1, 4, 11, 5, 9, 14) # THEO MOUSE
# clusters.groups <- c(unname(unlist(Reduce(intersect, clusters.groups))))
# clusters.groups <- clusters.groups[clusters.groups != 1]

colPal <- list("darkgrey", "#ff0000", "#ff8c00", "#ffd700", "#0000cd", "#00ff00", "#00fa9a",
               "#4169e1", "#e9967a", "#00ffff", "#ff1493", "#d8bfd8", "#ff00ff",
               "#f0e68c", "#ee82ee", "#00bfff")

p1 <- ggplot(popDT[clusters.group %in% clusters.groups],
# p1 <- ggplot(popDT,
             aes(centroid_x, -centroid_y, color = as.factor(clusters.group))) +
             # aes(centroid_x, -centroid_y, color = as.factor(clusters.name))) +
  theme_classic() +
  plotThemeDark(angle = 0) +
  geom_point(size = 0.1, alpha = 1) +
  # geom_point(data = popDT[clusters.name == "Others"], size = 0.1, alpha = 1) +
  # geom_point(data = popDT[clusters.name != "Others"], size = 0.1, alpha = 1) +
  geom_point(data = popDT[clusters.group == 1 & clusters.group %in% clusters.groups], size = 0.1, alpha = 1) +
  geom_point(data = popDT[clusters.group != 1 & clusters.group %in% clusters.groups], size = 0.1, alpha = 1) +
  # scale_color_brewer(palette = "Set3", name = "Clusters") +
  # scale_color_manual(values = clusterColours, name = "pop") +
  scale_color_manual(values = colPal, name = "Clusters") +
...
ggsave(file.path(anaDir, "maps", paste0("clusters_group_", cciaObj$getUID(), ".png")), width = 32, height = 18, plot = p1)
# ggsave(file.path(anaDir, "maps", paste0("clusters_name_", cciaObj$getUID(), ".png")), width = 24, height = 18, plot = p1)
```


```{r}
# is there any spatial interaction?
spatialDT <- cciaObj$spatialDT(valueName = "default", uIDs = uIDs)

# join pops
# spatialDT[popDT[, c("uID", "label", "clusters.group")],
spatialDT[popDT[, c("uID", "label", "clusters.name")],
          on = c("uID", "to" = "label"),
          # pop.to := clusters.group]
          pop.to := clusters.name]
# spatialDT[popDT[, c("uID", "label", "clusters.group")],
spatialDT[popDT[, c("uID", "label", "clusters.name")],
          on = c("uID", "from" = "label"),
          # pop.from := clusters.group]
...
clustMarkers <- colnames(popDT)
# clustMarkers <- colnames(regionsDT)
clustMarkers <- clustMarkers[!clustMarkers %in% c(
  "label", "clusters", "clusters.id", "clusters.name", "clusters.group", "regions",
  "UMAP_1", "UMAP_2", "pop", "centroid_y", "centroid_x", colnames(exp.info))]
...
plotSizes <- list(
  # init = c(6, 10)
  # init = c(6, 10)
  # init = c(8, 12) # THEO Mouse
  init = c(8, 16) # THEO Human
  # regions = c(4, 10)
)
```
...
for (i in names(clust)) {
  ...
  markersToShow <- markersToShow[!startsWith(markersToShow, "flow.")]
  ...
  for (j in unique(popDT$uID)) {
    y <- popDT[uID == j]
    
    colsToShow <- c("label", "clusters", "clusters.name", "clusters.group", "pop", "regions", markersToShow)
    colsToShow <- colsToShow[colsToShow %in% colnames(y)]
    # colsToShow <- colsToShow[colsToShow %in% colnames(regionsDT)]
    # colsToShow <- colsToShow[colsToShow %in% colnames(dfToPlot)]
    
    # show heatmap
    popLymphs <- y[pop %in% popsToShow, ..colsToShow]
    # popLymphs <- regionsDT[pop %in% popsToShow, ..colsToShow]
    ...
    # ggsave(file.path(anaDir, "heat", paste0("clusters_heat_", i, ".pdf")),
    ggsave(file.path(anaDir, "heat", paste0("clusters_heat_", exp.info[uID == j]$Treatment, "-", j, "-", i, ".pdf")),
...
# data.table::fwrite(summaryDF, file.path(anaDir, "pop_freq.csv"))
```
...
# save as csv for plotting in Prism
data.table::fwrite(summaryDF, file.path(anaDir, "regions_freq.csv"))
```
...
data.table::fwrite(datToPlot, file.path(anaDir, "Th_ae_contact.csv"))
```
...
data.table::fwrite(datToPlot, file.path(anaDir, "Th_ae_dist.csv"))
```
...
data.table::fwrite(datToPlot, file.path(anaDir, "Th_ae_dist_ratio.csv"))
```
...
for (i in unique(freqRegions$pop.from)) {
  ...
  # save as csv for plotting in Prism
  data.table::fwrite(x, file.path(anaDir, "interactions", paste0("interactions_freq_", i, ".csv")))
}
```
