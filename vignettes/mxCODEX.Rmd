...
cciaObj <- initCciaObject(
  pID = pID, uID = "1I5oli", versionID = versionID, initReactivity = FALSE # Set
)

channelNames <- sapply(
  xml2::xml_attrs(xml2::xml_find_all(
    cciaObj$omeXML(), cciaObj$omeXMLPath("//Image//Pixels//Channel"))),
  function(x) {x[[2]]})
...
cciaObj$setImFilepath("ccidAfCorrected.zarr", "afCorrected")
cciaObj$saveState()
```
...
cciaObj <- initCciaObject(
  # pID = pID, uID = "1I5oli", versionID = versionID, initReactivity = FALSE # Set
  pID = pID, uID = "YuX8GU", versionID = versionID, initReactivity = FALSE # Set
  # pID = pID, uID = "Ksflg7", versionID = versionID, initReactivity = FALSE # Set
)

imChannels <- unlist(cciaObj$imChannelNames())
numChannels <- length(imChannels)
seqChannels <- seq(numChannels) - 1
names(seqChannels) <- seqChannels

# prepare af combinations
afCombinations <- lapply(seqChannels, function(x) {
  list(
...
channelNames <- sapply(cciaObj$cciaObjects(), function(x) unname(x$imChannelNames()))
...
ggsave(file.path(anaDir, "umap.tiff"), width = 4, height = 4)
# ggsave(file.path(anaDir, "umap_coating.tiff"), width = 2, height = 2)
# ggsave(file.path(anaDir, "umap_coating_treatment.tiff"), width = 4, height = 2.5)
```
...
for (i in unique(dfToPlot$uID)) {
  ...
  ggsave(file.path(anaDir, paste0("clusters_", i, ".tiff")), width = 8, height = 8, plot = p1)
}
```
