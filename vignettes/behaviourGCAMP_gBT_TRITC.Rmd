...
funParams <- list(
  ...
  modelMeasurements = c(
    ...
    # "live.cell.peak.norm.GCAMP", "live.cell.peak.ratio.GCAMP"),
  colName = "gcamp",
  appendStates = list(),
  skipTimesteps = 0,
...
data.table::fwrite(exp.info[Focus == "non-clustering" & Cells == "gBT_TRITC" & Include == "Y"],
...
tracksInfo <- cciaObj$tracksInfo(
  "live.cell.hmm.state.gcamp", parentPop = "tcells.gBT/tracked",
  uIDs = uIDs, replaceNA = TRUE)
# tracksMeasures <- cciaObj$tracksMeasures(pops = "tcells.gBT/tracked", uIDs = uIDs)
```


```{r fig_GCAMP_hmm, fig.height=3, fig.width=10}
ggplot(
  popDTs[uID == "INw0Gd" & track_id == 3, ],
  aes(x = (centroid_t))) +
  geom_line(aes(y = live.cell.hmm.state.gcamp), colour = "black") +
...
```{r fig_hmm_freq, fig.height=4, fig.width=4}
# show HMM frequencies
datToPlot <- as.data.table(popDTs %>%
  group_by(uID, live.cell.hmm.state.gcamp) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n) * 100) %>%
  ungroup() %>%
  complete(uID, live.cell.hmm.state.gcamp, fill = list(freq = 0)) %>%
  dplyr::filter(live.cell.hmm.state.gcamp == 1) %>%
  left_join(exp.info))

ggplot(datToPlot,
# ggplot(dfToPlot %>% dplyr::filter(live.cell.hmm.state.gcamp.2.n >= 2) %>% left_join(exp.info),
       aes(interaction(Treatment, DTx), freq, color = interaction(Treatment, DTx))) +
       # aes(interaction(Treatment, DTx), log(live.cell.hmm.state.gcamp.2.diff), color = interaction(Treatment, DTx))) +
  theme_classic() +
...
t.A <- t.test(
  datToPlot[Treatment == "PBS" & DTx == "PBS"]$freq,
  datToPlot[Treatment == "HSV" & DTx == "PBS"]$freq
)
t.B <- t.test(
  datToPlot[Treatment == "PBS" & DTx == "PBS"]$freq,
  datToPlot[Treatment == "HSV" & DTx == "DTx"]$freq
)

print(paste("PBS-HSV", round(t.A$p.value, digits = 4)))
print(paste("PBS-DTx", round(t.B$p.value, digits = 4)))
...
# dfToPlot[dfToPlot$live.cell.hmm.state.gcamp.2.diff > 0, ]$gcamp.track <- TRUE
# dfToPlot[dfToPlot$live.cell.hmm.state.gcamp.2.diff < 100 &
#            dfToPlot$live.cell.hmm.state.gcamp.2.n >= 2, ]$gcamp.track <- TRUE

print(table(dfToPlot$gcamp.track))
...
ggplot(
  ...
  geom_point(aes(color = live.cell.hmm.state.gcamp.1), size = 1) +
  viridis::scale_color_viridis() + 
  theme(
    axis.text = element_text(size = 15),
    axis.text.x = element_blank(),
...
# data.table::fwrite(summaryDF, file = file.path(anaDir, "freq_clusters_wo_clusters.csv"))

# t.A <- t.test(
#   summaryDF[Treatment == "PBS" & DTx == "PBS" & clusters == 2]$freq.clusters,
#   summaryDF[Treatment == "HSV" & DTx == "PBS" & clusters == 2]$freq.clusters
# )
# t.B <- t.test(
#   summaryDF[Treatment == "PBS" & DTx == "PBS" & clusters == 2]$freq.clusters,
#   summaryDF[Treatment == "HSV" & DTx == "DTx" & clusters == 2]$freq.clusters
# )
# 
# print(paste("PBS-HSV", round(t.A$p.value, digits = 4)))
# print(paste("PBS-DTx", round(t.B$p.value, digits = 4)))
