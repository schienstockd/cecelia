...
ggplot(
  ...
  theme_classic() +
  geom_point(data = datToPlot %>% dplyr::filter(gcamp.track == FALSE),
             color = "#B3BCC2", size = 2) +
  geom_point(data = datToPlot %>% dplyr::filter(gcamp.track == TRUE),
...
ggsave(file.path(anaDir, paste0("umap_GCAMP.tiff")), width = 5, height = 5)
```
...
ggsave(file.path(anaDir, "clusters_heat.pdf"), width = 6, height = 4)
```
