...
```{r fig_umap, fig.height=2, fig.width=2}
# GCAMP
tracksInfo <- cciaObj$tracksInfo("live.cell.hmm.state.gcamp", parentPop = "tcells.gBT/tracked", uIDs = uIDs)

dfToPlot <- copy(tcells.sc) %>% drop_na(clusters) %>% left_join(tracksInfo)
dfToPlot$gcamp.track <- FALSE
dfToPlot[dfToPlot$live.cell.hmm.state.gcamp.2.n >= 2, ]$gcamp.track <- TRUE
...
```
```{r fig_GCAMP_behaviour, fig.height=4, fig.width=4}
...
datToPlot <- dfToPlot %>%
  ...
  group_by(Treatment, DTx, gcamp.track, clusters.name) %>%
...
datToPlot$gcamp.track <- case_match(
  datToPlot$gcamp.track, c(TRUE) ~ ">= 2", c(FALSE) ~ "< 2")

colPal <- randomcoloR::distinctColorPalette(length(unique(dfToPlot$clusters)))

# ggplot(datToPlot, aes(interaction(Treatment, DTx), freq,
# ggplot(datToPlot, aes(gcamp.track, freq, fill = clusters)) +
# ggplot(datToPlot, aes(interaction(Treatment, DTx), freq, fill = clusters)) +
ggplot(datToPlot, aes(paste(Treatment, DTx), freq, fill = clusters.name)) +
  theme_classic() +
  ...
  xlab("") + ylab("") +
  # facet_grid(.~interaction(uID, Treatment, DTx)) +
  # facet_grid(.~interaction(Treatment, DTx)) +
  facet_grid(.~gcamp.track) +
...
```{r fig_GCAMP_behaviour, fig.height=3, fig.width=8}
# in total - what is the behaviour of GCAMP+?
# datToPlot <- .mapClustNames(dfToPlot, clusterMapping, "clusters")
...
datToPlot <- dfToPlot %>%
  ...
  # group_by(uID, clusters, gcamp.track) %>%
  group_by(uID, gcamp.track, clusters.name) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n) * 100) %>%
  ungroup() %>%
  # complete(uID, gcamp.track, clusters, fill = list(freq = 0)) %>%
  complete(uID, gcamp.track, clusters.name, fill = list(freq = 0)) %>%
  dplyr::filter(gcamp.track == TRUE) %>% 
  left_join(exp.info)

# ggplot(dfToPlot %>% left_join(exp.info),
ggplot(datToPlot,
       # aes(clusters, freq, color = interaction(Treatment, DTx))) +
       aes(clusters.name, freq, color = interaction(Treatment, DTx))) +
       # aes(clusters, live.cell.hmm.state.gcamp.3.n, color = interaction(Treatment, DTx))) +
       # aes(clusters, live.cell.hmm.state.gcamp.3, color = interaction(Treatment, DTx))) +
  theme_classic() +
  geom_boxplot(outlier.alpha = 0) +
...
datToPlot$cell.id <- 0
datToPlot[datToPlot$clusters.name == "Meandering",]$cell.id <- 1
datToPlot[datToPlot$clusters.name == "Scanning",]$cell.id <- 2
datToPlot[datToPlot$clusters.name == "Clustering",]$cell.id <- 3

# ggsave(file.path(anaDir, paste0("GCAMP_behaviour.tiff")), width = 8, height = 2)
write.csv(datToPlot, file = file.path(anaDir, "freq_gcamp_behaviour.csv"))
```
...
ggsave(file.path(anaDir, "clusters_heat.pdf"), width = 2.8, height = 2.8)
```
