...
funParams <- list(
  ...
  postFiltering = 2,
  postIterations = 2,
  # numStates = 4, # movement with injection
  numStates = 4, # shape
  uIDs = uIDs
)
...
# plot UMAP
curDatToPlot <- datToPlot[datToPlot$Treatment != "HSV",]
...
curDatToPlot <- datToPlot[datToPlot$Treatment == "HSV" & datToPlot$DTx == "PBS",]
plotCols <- .flowColours(curDatToPlot$UMAP_1, curDatToPlot$UMAP_2, colramp = viridisLite::inferno)
...
curDatToPlot <- datToPlot[datToPlot$Treatment == "HSV" & datToPlot$DTx == "DTx",]
plotCols <- .flowColours(curDatToPlot$UMAP_1, curDatToPlot$UMAP_2, colramp = viridisLite::inferno)
...
```{r fig_treatment_pseudotime, fig.height=3, fig.width=6}
dfToPlot <- as.data.table(copy(tcells.sc) %>% drop_na(clusters) %>% left_join(exp.info))

ggplot(dfToPlot,
       aes(interaction(Treatment, DTx), dpt_pseudotime, color = interaction(Treatment, DTx))) +
  theme_classic() +
  # geom_boxplot(outlier.alpha = 0) +
  geom_violin(scale = "width") +
  geom_jitter(
    position = position_jitterdodge(jitter.width = 1), alpha = 0.1) 
    # width = 2, alpha = 1.0) 
