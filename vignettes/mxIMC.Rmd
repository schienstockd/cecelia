...
```{r fig_mean_intensity_radii, fig.height=1, fig.width=4}
library(tidyverse)
library(readr)
library(dbscan)
library(ggplot2)
df <- read.csv(file = "~/Downloads/DK01P_nonbrisk.csv", na = c("", "NA")) %>%
  drop_na()
colnames(df) <- c("Image", "Class", "coord_x", "coord_y", "CD31",
                  "MITF", "NGFR", "CD3",
                  "CD20", "SOX10")

x <- as.matrix(df[df$Class == "SOX10", c("coord_x", "coord_y")])
y <- as.matrix(df[df$Class != "SOX10", c("coord_x", "coord_y")])

rownames(x) <- rownames(df[df$Class == "SOX10", ])
rownames(y) <- rownames(df[df$Class != "SOX10", ])

# get CD3 intensity for radii
mean.intensities <- c()
dists <- seq(30, 100, 5)
dists.CD3 <- c()
dists.CD20 <- c()
...
for (i in dists) {
  if (i %% 10 == 0)
    print(paste(">> Get frNN for", i))
  x.neighbours <- dbscan::frNN(x, eps = i, query = y)
  
  # get labels
  x.rows <- rownames(df) %in% rownames(y)[unique(unlist(x.neighbours$id))]
  
  # get average CD3 intensity
  dists.CD3 <- c(dists.CD3, mean(df[x.rows, ]$CD3))
  dists.CD20 <- c(dists.CD20, mean(df[x.rows, ]$CD20))
  }
...
ggplot(df2, aes(dist, value)) +
  theme_classic() +
  geom_smooth() +
  xlab("Radius (um)") + ylab("Mean intensity (AU)") +
