...
cropX <- 256
max.dist <- 2
# nnY <- shgDTs[centroid_x < cropX, c('centroid_z', 'centroid_y', 'centroid_x')]
nnY <- shgDTs[centroid_x < cropX, c('centroid_y', 'centroid_x')]

# for (i in unique(popDTs$pop)) {
hmmStates <- unique(popDTs$live.cell.hmm.state.movement)
for (i in hmmStates[!is.na(hmmStates)]) {
  # x <- popDTs[pop == i]
  x <- popDTs[centroid_x < cropX & live.cell.hmm.state.movement == i]
  
  x$colour <- .flowColours(x$centroid_x, x$centroid_y)
  
  # get overlapping cells with SHG
  # nnX <- x[, c('centroid_z', 'centroid_y', 'centroid_x')]
  nnX <- x[, c('centroid_y', 'centroid_x')]
  
  shgNN <- dbscan::kNN(nnY, k = 1, query = nnX)
  x[, shgDist := shgNN$dist]
  x[, shgContact := shgDist < max.dist]
  
  print(x %>% group_by(shgContact) %>% summarise(n = n()) %>% mutate(freq = n/sum(n) * 100))
  
  p1 <- ggplot(
    x, aes(centroid_x, -centroid_y)) +
    theme_classic() +
    geom_point(data = shgDTs[centroid_x < cropX], color = "white") +
