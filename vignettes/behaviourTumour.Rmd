...
for (x in cciaObj$cciaObjects(uIDs = uIDs)) {
  channelAttr <- attributes(x$imChannelNames())
  channelAttr$names <- channelAttr$names[1:4]
  channelNames <- x$imChannelNames()[1:4]
  attributes(channelNames) <- channelAttr
  x$setImChannelNames(channelNames, valueName = "corrected")
  x$saveState()
}
```
