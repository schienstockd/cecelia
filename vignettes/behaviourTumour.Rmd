...
cciaObj <- initCciaObject(
  ...
  pID = pID, uID = "iEzpp6", versionID = versionID, initReactivity = FALSE # Tumour
)
...
# hmmTime$hmm.type <- "0"
# hmmTime[hmmTime$live.cell.hmm.state.movement %in% c(2), ]$hmm.type <- "1"
# hmmTime[hmmTime$live.cell.hmm.state.movement %in% c(3), ]$hmm.type <- "2"
# hmmTime[hmmTime$live.cell.hmm.state.movement %in% c(4), ]$hmm.type <- "3"
# hmmTime$hmm.type <- factor(hmmTime$hmm.type, levels = c("0", "1", "2", "3"))
...
ggplot(hmmTime,
  ...
  xlab("Time (min)") + ylab("Velocity (um/min)") +
  facet_grid(uID~.) +
