...
# get branching
labels <- lapply(cciaObj$cciaObjects(uIDs = uIDs), function(x) {
  y <- x$labelProps(valueName = "SHG.branch")
  uns <- y$values_uns()
  y$close()
...
# tracks <- cciaObj$tracks(pop = pops$gBT, uIDs = uIDs, steps.subtracks = 4)

# get tracks info and check parameters
tracksInfo <- cciaObj$tracksMeasures(
  pops = pops[1], uIDs = uIDs, measures = c(),
  # extraMeasures = c("tracks.coords"), steps.subtracks = 4)
...
pos <- lapply(tracks$fYyXay, function(x) {
  posHead <- head(x, 1)
  posTail <- tail(x, 1)
