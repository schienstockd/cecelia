...
hmmSpace <- copy(popDTs) %>%
  dplyr::filter(
    !is.na(live.cell.track.clusters.default),
        live.cell.track.clusters.default != "NA"
    )
...
for (i in unique(hmmSpace$uID)) {
  for (j in unique(hmmSpace$live.cell.track.clusters.default)) {
    x <- hmmSpace[hmmSpace$uID == i & hmmSpace$live.cell.track.clusters.default == j,]
    
    hmmSpace[hmmSpace$uID == i & hmmSpace$live.cell.track.clusters.default == j,]$density <- .flowColours(
...
ggplot(hmmSpace, aes(centroid_x, centroid_y)) +
  ...
  # geom_point(color = hmmSpace$density) +
  geom_point(
    # aes(color = live.cell.track.clusters.default),
  ...
  facet_grid(live.cell.track.clusters.default~uID)

ggsave(file.path(anaDir, "cluster_space.tiff"), height = 5, width = 2)
```