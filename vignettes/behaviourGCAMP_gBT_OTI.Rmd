...
```{r fig_GCAMP_behaviour, fig.height=3, fig.width=5}
...
datToPlot <- dfToPlot %>%
  ...
  group_by(pop, gcamp.track, clusters) %>%
  ...
  # dplyr::filter(gcamp.track == TRUE)
  # ungroup() %>%
  # complete(Treatment, DTx, gcamp.track, clusters, fill = list(freq = 0))

datToPlot$gcamp.track <- case_match(
  datToPlot$gcamp.track, c(TRUE) ~ ">= 2", c(FALSE) ~ "0")

# colPal <- randomcoloR::distinctColorPalette(length(unique(dfToPlot$clusters)))

datToPlot$pop <- case_match(
  as.character(datToPlot$pop),
  c("tcells.gBT/tracked") ~ "gBT-I",
  c("tcells.OTI/tracked") ~ "OT-I"
)

# ggplot(datToPlot, aes(interaction(Treatment, DTx), freq,
ggplot(datToPlot, aes(gcamp.track, freq, fill = clusters)) +
  theme_classic() +
  ...
  xlab("GCAMP peaks") + ylab("") +
  facet_grid(.~pop) +
  guides(fill = guide_legend(ncol = 2))

ggsave(file.path(anaDir, "clusters_GCAMP_behaviour.pdf"), width = 3.5, height = 2.5)

# datToPlot$Treatment.DTx <- -1
