...
cciaObj <- initCciaObject(
  ...
  pID = pID, uID = "yl78XL", versionID = versionID, initReactivity = FALSE #  GCAMP
)

cciaObj$imLabelsFilepath()
...
#   x$setImChannelNames(
#     c("gBT-CTDR", "AF", "GCAMP", "OTI-CTV", "Sliding GCAMP"), valueName = "slidingWindow")
#   x$saveState()
# }
```
...
funParams <- list(
  ...
  # modelMeasurements = c("live.cell.peak.ratio.GCAMP", "live.cell.peak.norm.GCAMP"),
  modelMeasurements = c(
    "live.cell.speed", "live.cell.angle", "live.cell.peak.ratio.GCAMP"),
  colName = "gcamp",
  appendStates = list(),
  # skipTimesteps = 2,
  skipTimesteps = 0,
...
data.table::fwrite(exp.info[Focus == "non-clustering" & Cells == "gBT_OTI" & Include == "Y"],
                   file.path(anaDir, "exp_info.csv"))
...
tcells.sc.info <- as.data.table(
  copy(tcells.sc) %>% left_join(tracks.info, by = c("uID", "pop", "track_id")))
...
# ggsave(file.path(anaDir, "umap_wo_label.png"), width = 2, height = 2)
# ggsave(file.path(anaDir, "umap_grouped.png"), width = 2, height = 2)
# ggsave(file.path(anaDir, "umap_wo_label_grouped.png"), width = 2, height = 2)
```
...
# plot UMAP
plot.p1(datToPlot, datToPlot %>% dplyr::filter(pop == "tcells.gBT/tracked"))
...
dfToPlot <- copy(tcells.sc.info)
dfToPlot$gcamp.track <- FALSE
# dfToPlot[dfToPlot$live.cell.hmm.state.gcamp.1.n >= 2, ]$gcamp.track <- TRUE
dfToPlot[dfToPlot$live.cell.hmm.state.gcamp.3.n >= 3, ]$gcamp.track <- TRUE
...
# data.table::fwrite(datToPlot, file = file.path(anaDir, "freq_gcamp.csv"))
```

```{r fig_GCAMP_behaviour, fig.height=4, fig.width=6}
# in total - what is the behaviour of GCAMP+?
# datToPlot <- .mapClustNames(dfToPlot, clusterMapping, "clusters")

# dfToPlot <- .mapClustNames(dfToPlot, clusterMapping)

datToPlot <- dfToPlot %>%
  left_join(exp.info) %>%
  # dplyr::filter(Treatment != "PBS") %>%
  # dplyr::filter(clusters != 5) %>%
  # group_by(Treatment, DTx, gcamp.track, clusters) %>%
  # group_by(uID, Treatment, DTx, gcamp.track, clusters.name) %>%
  # group_by(uID, pop, clusters.name, gcamp.track) %>%
  group_by(uID, pop, gcamp.track, clusters) %>%
  # group_by(uID, pop, clusters, gcamp.track) %>%
  # group_by(uID, Treatment, DTx, gcamp.track, clusters) %>%
  # group_by(gcamp.track, clusters) %>%
  # group_by(gcamp.track, clusters.name) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n) * 100) %>%
  dplyr::filter(gcamp.track == TRUE)
  # ungroup() %>%
  # complete(uID, pop, clusters.name, gcamp.track, fill = list(freq = 0))

ggplot(datToPlot,
       # aes(clusters.name, freq, color = pop)) +
       aes(clusters, freq, color = pop)) +
  theme_classic() +
  geom_boxplot(outlier.alpha = 0) +
  geom_jitter(
    position = position_jitterdodge(jitter.width = 0.10), alpha = 1.0) 
...
# ggsave(file.path(anaDir, "clusters_GCAMP_behaviour.pdf"), width = 2, height = 2.5)

# datToPlot$clust.ID <- -1
# datToPlot[datToPlot$clusters.name == "Directed",]$clust.ID <- 0
# datToPlot[datToPlot$clusters.name == "Meandering",]$clust.ID <- 1
# datToPlot[datToPlot$clusters.name == "Scanning",]$clust.ID <- 2
# datToPlot[datToPlot$clusters.name == "Aggregating",]$clust.ID <- 3
# 
# datToPlot$pop.ID <- 0
# datToPlot[datToPlot$pop == "tcells.OTI/tracked",]$clust.ID <- 1
# 
# data.table::fwrite(datToPlot, file = file.path(anaDir, "freq_gcamp_behaviour_total.csv"))
```
...
datToPlot <- popDTs %>%
  mutate(live.cell.track.clusters.hmm_transitions_movement_OTI = as.numeric(as.character(
    live.cell.track.clusters.hmm_transitions_movement_OTI))) %>%
  drop_na(live.cell.track.clusters.hmm_transitions_movement_OTI) %>%
  drop_na(live.cell.hmm.state.movement) %>%
  group_by(live.cell.track.clusters.hmm_transitions_movement_OTI, live.cell.hmm.state.movement) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n) * 1) %>%
  ungroup() %>%
  complete(live.cell.track.clusters.hmm_transitions_movement_OTI, live.cell.hmm.state.movement, fill = list(freq = 0))

data.table::fwrite(datToPlot, file = file.path(anaDir, "freq_hmm.csv"))

ggplot(datToPlot, aes(as.factor(live.cell.track.clusters.hmm_transitions_movement_OTI), freq,
...
datToPlot <- popDTs %>%
  mutate(
    live.cell.track.clusters.hmm_transitions_movement_OTI = as.numeric(as.character(
      live.cell.track.clusters.hmm_transitions_movement_OTI)),
  ...
  # group_by(live.cell.track.clusters.hmm_transitions_movement_OTI, from, to) %>%
  # dplyr::filter(!live.cell.hmm.transitions.movement %in% paste0(seq(4), "_", seq(4))) %>%
  group_by(live.cell.track.clusters.hmm_transitions_movement_OTI, live.cell.hmm.transitions.movement) %>%
  summarise(n = n()) %>%
  mutate(freq = n / sum(n) * 1) %>%
  separate_wider_delim(
    live.cell.hmm.transitions.movement, delim = "_", names = c("from", "to"))
  # ungroup() %>%
  # complete(live.cell.track.clusters.hmm_transitions_movement_OTI, from, to, fill = list(freq = 0))
...
ggplot(datToPlot, aes(as.factor(from), as.factor(to), size = freq * 2, fill = freq)) +
  ...
  facet_wrap(.~live.cell.track.clusters.hmm_transitions_movement_OTI, nrow = 2)

ggsave(file.path(anaDir, "clusters_transitions_heat.pdf"), width = 6, height = 4)
```
...
summaryDF <- as.data.table(summaryDF[, .(n.clusters = .N), by = .(uID, pop, clusters)] %>%
# summaryDF <- summaryDF[clusters != 6, .(n.clusters = .N), by = .(uID, pop, clusters)] %>%
...
t.A <- t.test(
  summaryDF[pop == "tcells.OTI/tracked" & clusters == 3]$freq.clusters,
  summaryDF[pop == "tcells.gBT/tracked" & clusters == 3]$freq.clusters, paired = TRUE
)

print(paste("PBS-HSV", round(t.A$p.value, digits = 4)))

# ggsave(file.path(anaDir, "clusters_freq.pdf"), width = 8, height = 3)
# 
# summaryDF$cell_ID <- 0
# summaryDF[summaryDF$pop == "tcells.gBT/tracked",]$cell_ID <- 0
# summaryDF[summaryDF$pop == "tcells.OTI/tracked",]$cell_ID <- 1

data.table::fwrite(
  summaryDF,
  file = file.path(anaDir, "freq_clusters.csv")
  # file = file.path(anaDir, "freq_clusters_wo_clusters.csv")
)
```

```{r fig_GCAMP_behaviour, fig.height=4, fig.width=8}
# in total - what is the behaviour of GCAMP+?
# datToPlot <- .mapClustNames(dfToPlot, clusterMapping, "clusters")
...
datToPlot <- dfToPlot %>%
  ...
  # group_by(Treatment, DTx, gcamp.track, clusters) %>%
  # group_by(uID, Treatment, DTx, gcamp.track, clusters.name) %>%
  # group_by(uID, clusters.name, gcamp.track) %>%
  group_by(uID, pop, clusters, gcamp.track) %>%
  # group_by(uID, gcamp.track, clusters) %>%
  # group_by(uID, gcamp.track, clusters.name) %>%
  # group_by(uID, clusters.name, gcamp.track) %>%
  # group_by(uID, Treatment, DTx, gcamp.track, clusters) %>%
  # group_by(gcamp.track, clusters) %>%
  # group_by(gcamp.track, clusters.name) %>%
  ...
  dplyr::filter(gcamp.track == TRUE) %>%
  ungroup() %>%
  # complete(uID, clusters.name, gcamp.track, fill = list(freq = 0))
  # complete(uID, clusters, gcamp.track, fill = list(freq = 0))
  complete(uID, pop, gcamp.track, clusters, fill = list(freq = 0))
  # complete(uID, gcamp.track, clusters.name, fill = list(freq = 0))

ggplot(datToPlot %>% left_join(exp.info),
       # aes(clusters.name, freq)) +
       aes(clusters, freq, color = pop)) +
       # aes(clusters.name, freq, color = interaction(Treatment, DTx))) +
  theme_classic() +
  geom_boxplot(outlier.alpha = 0) +
  geom_jitter(
    position = position_jitterdodge(jitter.width = 0.10), alpha = 1.0)
...
# ggsave(file.path(anaDir, "clusters_GCAMP_behaviour.png"), width = 5, height = 2.5)
# ggsave(file.path(anaDir, "clusters_behaviour_GCAMP.png"), width = 5, height = 2.5)
...
datToPlot <- dfToPlot %>%
  ...
  # complete(uID, clusters.name, gcamp.track, fill = list(freq = 0))
  # complete(uID, clusters, gcamp.track, fill = list(freq = 0))
  complete(uID, pop, clusters, gcamp.track, fill = list(freq = 0))
  # complete(uID, gcamp.track, clusters.name, fill = list(freq = 0))
...
# popDTs %>%
  ...
  group_by(uID, pop, live.cell.hmm.state.gcamp, live.cell.track.clusters.hmm_transitions_movement_OTI) %>%
  summarise(n.clust = n()) %>%
  mutate(freq.clust = n.clust/sum(n.clust) * 100) %>%
  ungroup() %>%
  complete(
    # uID, pop, live.cell.hmm.state.gcamp, live.cell.track.clusters.skip3,
    uID, pop, live.cell.hmm.state.gcamp, live.cell.track.clusters.hmm_transitions_movement_OTI,
    fill = list(freq.clust = 0)
    ) %>%
  dplyr::filter(
    # !live.cell.track.clusters.skip3 %in% c("NA", NA),
    !live.cell.track.clusters.hmm_transitions_movement_OTI %in% c("NA", NA),
...
ggplot(summaryDF,
       aes(
         ...
         fill = as.factor(live.cell.track.clusters.hmm_transitions_movement_OTI)
         )) +
  theme_classic() +
  geom_boxplot(outlier.alpha = 0) +
  geom_jitter(
