...
write.csv(summaryDF %>% arrange(live.cell.track.clusters.hmm_transitions_movement),
...
ggplot(speedTimeClusters %>%
  ...
  stat_smooth(span = 0.8, alpha = 0.2) +
  theme(
    legend.title = element_blank(),
    legend.position = "None",
    strip.background = element_blank(),
...
# ggplot(hmmTime[hmmTime$uID == i,],
ggplot(hmmTime,
       aes((centroid_t * time.interval), freq,
           color = clusters.name,
           fill = clusters.name
           )) +
  stat_smooth(size = 1, span = 0.2, alpha = 0.2) +
  # geom_line(size = 0.8) +
  theme_classic() +
  scale_color_brewer(name = NULL, palette = "Set1") +
