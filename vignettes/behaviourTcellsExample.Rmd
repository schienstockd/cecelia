...
for (i in seq(8)) {
  tracks.gBT.dup <- copy(tracks.gBT.sim)
  
  # Add some noise to coordinates
  for (i in names(tracks.gBT.dup)) {
    ...
    tracks.gBT.dup[[i]][,"z"] <- tracks.gBT.dup[[i]][,"z"] + rnorm(nrow(tracks.gBT.dup[[i]]), sd = 0.5)
  }
  
  # Wrap the track in a tracks object and add it to the TCell data with
  # a unique id number
  max.id <- max(as.numeric(names(tracks.gBT.sim)))
  new.ids <- seq(max.id + 1, max.id + length(tracks.gBT.sim))
