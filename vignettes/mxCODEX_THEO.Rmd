...
marker.def <- as.data.table(read.csv(file.path(anaDir, "Mouse_CODEX Neuro Panel_Biomarkers_Cell Type.csv")))

marker.def[, desc := paste(Cell.Type.level.1, Cell.Type.level.2, sep = ">")]
# marker.def[, desc := Cell.Type.level.1]
setnames(marker.def, "X", "marker")

marker.def <- marker.def[, c("marker", "desc")]
marker.def[, marker := .flowCorrectChannelNames(marker)]

# remove trailing separator
marker.def[, desc := stringr::str_replace(desc, ">$", "")]
```
...
# pre-create populations to be added for each image
# then go through and define them
popNames <- unique(marker.def$desc)

# get colours
popColours <- randomcoloR::distinctColorPalette(length(popsToAdd))
popsToAdd <- lapply(popColours, function(x) {
  list(
...
names(popsToAdd) <- popNames

# add populations
x$delPopsByPath("clust", pops = names(popsToAdd), includeFiltered = TRUE)
x$addFilteredPops("clust", "root", popsToAdd, valueName = "default")
x$saveState()

# then propagate
cciaObj$propagatePopMap("clust", x$getUID(), uIDs[uIDs != x$getUID()], saveState = TRUE)
```
...
plotSizes <- list(
  # init = c(6, 10)
  # init = c(6, 10)
  init = c(14, 16) # THEO Mouse
  # init = c(14, 16) # THEO Human
  # regions = c(4, 10)
)
```


```{r fig_clusters_heat, fig.height=10, fig.width=10}
for (i in names(clust)) {
  ...
  for (j in unique(popDT$uID)) {
    ...
    markersToPlot <- popLymphs %>%
      ...
      left_join(marker.def) %>%
      mutate(marker = paste(desc, marker))
