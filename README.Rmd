...
We incorporated [`btrack`](https://github.com/quantumjot/BayesianTracker/tree/38b144c09f384cb30c2fd9572f19f8dfb3007fca) to track segmented cells.
...
We incorporated `Leiden clustering` to cluster tracks based on `whole` track measurements
and individual `object` measurements. These clusters can be compared in time which
shows that cells first have `directed` movement. After treatment, they `stop` and return
to `meandering` behaviour.
